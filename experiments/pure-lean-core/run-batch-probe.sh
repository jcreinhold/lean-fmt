...
probe_exe=${PURE_LEAN_PROBE_EXE:-pure-lean-core}
probe_mode=${PURE_LEAN_PROBE_MODE:-full}
result_dir="$experiment_root/results"
mkdir -p "$result_dir"
stamp=$(date -u +%Y%m%dT%H%M%SZ)
output="$result_dir/batch-probe-$stamp.txt"
meta="$result_dir/batch-probe-$stamp.meta"

files=()
if (($#)); then
  for file in "$@"; do
    files+=("$mathlib_root/$file")
  done
else
  while IFS= read -r file; do
    [[ -z $file ]] || files+=("$mathlib_root/$file")
...
started=$(date +%s)
mode_args=()
if [[ $probe_mode == import ]]; then
  mode_args+=(--import-only)
fi
LEAN_NUM_THREADS=1 perl -MPOSIX -e \
  'POSIX::setpgid(0, 0) or die "setpgid: $!"; exec @ARGV or die "exec: $!"' \
  lake exe "$probe_exe" "${mode_args[@]}" "$mathlib_root" "${files[@]}" >"$output" 2>>"$meta" &
pid=$!
peak=0
stopped=0
