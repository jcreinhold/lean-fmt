#!/usr/bin/env bash
# RRE-SPEC. Characterize the rule boundary this stack is about to redesign.
#
# Two questions, both answered against the live product:
#
#   1. Rule enablement has two spellings — `--ignore FMT001` (a RulePlan projection) and
#      `leanFmt.trailingWhitespace=false` (a traced Lean option baked into the module artifact).
#      Do they agree with each other, and does each agree with itself across modes?
#   2. `LeanFmt/CompilerPlugin.lean` imports `LeanFmt.Rules`, and every plugin-using module lists
#      the plugin as a Lake dependency. Is one rule's message text therefore part of the compiled
#      bytes of every module in the target project?
#
...
echo "=== 1. two spellings of \"turn FMT001 off\" ==="
add_trailing_whitespace
echo
echo '--- leanFmt.trailingWhitespace=true (default): the artifact carries FMT001'
...
echo
echo '--- leanFmt.trailingWhitespace=false: the artifact omits FMT001'
LEAN_NUM_THREADS=1 lake -R -KleanFmtTrailingWhitespace=false build \
  lean-fmt +LocalSyntax:leanFmtArtifact >/dev/null 2>&1
echo '$ lean-fmt-tests verify-official-facet . tests/compiler/LocalSyntax.lean false'
...
for mode in check format; do
  rm -rf "$cache_root"
  echo "\$ lean-fmt $mode --no-cache --json tests/compiler/LocalSyntax.lean"
  LEAN_NUM_THREADS=1 lake -KleanFmtTrailingWhitespace=false exe lean-fmt "$mode" \
    --no-cache --json tests/compiler/LocalSyntax.lean | summarize || true
done
