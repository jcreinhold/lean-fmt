...
private def testUsageRejections (ctx : Ctx) (dirty : System.FilePath) : IO Unit := do
  let check (label : String) (args : List String) (expected : String) (code : UInt32 := 2) :
    IO Unit := do
...
private def testFormatStreams (ctx : Ctx) (dirty : System.FilePath) : IO Unit := do
  let formatted ←
    expectExit 0 "format -" ctx.app #["format", "-", "--stdin-filename", ctx.identity] (input? :=
        some (← IO.FS.readFile dirty)) (cwd? := some ctx.root)
  ...
  discard <|
      expectExit 0 "format - naming a real file" ctx.app
        #["format", "-", "--stdin-filename", "tests/fixtures/check/Layout.lean"] (input? :=
        some (← IO.FS.readFile dirty)) (cwd? := some ctx.root)
...
private def testOtherModes (ctx : Ctx) (dirty : System.FilePath) : IO Unit := do
  ...
  let check ←
    expectExit 0 "check -" ctx.app #["check", "-", "--stdin-filename", ctx.identity] (input? :=
  ...
  ensureEq "diff - emits a unified diff" s!"--- a/{ctx.identity}"
      ((diff.stdout.splitOn "\n").head?.getD "")
  let formatCheck ←
    expectExit 1 "format --check -" ctx.app
        #["format", "--check", "-", "--stdin-filename", ctx.identity, "--check"] (input? :=
        some source) (cwd? := some ctx.root)
...
private def testRangeExpansion (ctx : Ctx) (dirty : System.FilePath) : IO Unit := do
  ...
  let stream (args : Array String) (input : String := source) : IO ProcResult :=
    runProc ctx.app (#["format", "-", "--stdin-filename", ctx.identity] ++ args) (input? :=
      some input) (cwd? := some ctx.root)
  let ranged ←
    expectExit 0 "range" (ctx.app)
        (#["format", "-", "--stdin-filename", ctx.identity, "--range", "30:49"]) (input? :=
        some source) (cwd? := some ctx.root)
  ensureEq "a range formats its own unit" "namespace Alpha"
      ((ranged.stdout.splitOn "\n")[4]?.getD "<missing>")
  ensure (ranged.stdout.contains "def  x   :=   1") "a range rewrote the other command's bytes"
  ensureEq "the reported actual range is the unit, not the request"
      s!"{ctx.identity}: formatted range 30-51" (lastErr ranged)
  let formatted ←
    expectExit 0 "whole" ctx.app #["format", "-", "--stdin-filename", ctx.identity] (input? :=
        some source) (cwd? := some ctx.root)
  let full ←
    expectExit 0 "full range" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range", "0:78"] (input? :=
        some source) (cwd? := some ctx.root)
  ...
  let start :=
    ((jsonAt? map [.field "sourceMap", .index 0, .field "output", .field "start"]) |>.bind
            (·.getNum?.toOption)).getD
        0 |>.mantissa.toNat
  let stop :=
    ((jsonAt? map [.field "sourceMap", .index 0, .field "output", .field "stop"]) |>.bind
            (·.getNum?.toOption)).getD
        0 |>.mantissa.toNat
  let again ←
    expectExit 0 "again" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range", s!"{start}:{stop}"] (input? :=
        some ranged.stdout) (cwd? := some ctx.root)
...
private def testEncodings (ctx : Ctx) : IO Unit := do
  ...
  let rendered ←
    expectExit 0 "utf8 stream" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range-lines", "3:1-3:18"] (input? :=
        some (← IO.FS.readFile utf8)) (cwd? := some ctx.root)
  ...
  let roundTrip ←
    expectExit 0 "crlf stream" ctx.app #["format", "-", "--stdin-filename", ctx.identity] (input? :=
        some (← IO.FS.readFile crlf)) (cwd? := some ctx.root)
...
private def testJsonSurface (ctx : Ctx) (dirty : System.FilePath) : IO Unit := do
  let json ←
    parseJson
        (←
            fmt ctx #["format", "-", "--stdin-filename", ctx.identity, "--range", "30:49", "--json"]
                dirty).stdout
        "json"
  ...
  ensure ((formatted.splitOn "\n").contains "namespace Alpha") "json does not carry the bytes"
  let sourceMap := (json.getObjVal? "sourceMap").toOption.getD .null
  ensureEq "json carries the source map" 1 (sourceMap.getArr?.toOption.getD #[]).size
  -- The source map indexes the streamed text.
  let start :=
    ((jsonAt? json [.field "sourceMap", .index 0, .field "output", .field "start"]) |>.bind
            (·.getNum?.toOption)).getD
        0 |>.mantissa.toNat
  let stop :=
    ((jsonAt? json [.field "sourceMap", .index 0, .field "output", .field "stop"]) |>.bind
            (·.getNum?.toOption)).getD
        0 |>.mantissa.toNat
...
private def units (ctx : Ctx) (buffer : System.FilePath) : IO String := do
  let json ←
    parseJson (← fmt ctx #["format", "-", "--stdin-filename", ctx.identity, "--json"] buffer).stdout
        "units"
  let entries := ((json.getObjVal? "sourceMap").toOption.getD (.arr #[])).getArr?.toOption.getD #[]
  let spans :=
    entries.toList.map fun entry =>
      let spanAt (key : String) : Nat :=
        ((jsonAt? entry [.field "source", .field key]) |>.bind (·.getNum?.toOption)).getD
            0 |>.mantissa.toNat
...
private def testForwardExtension (ctx : Ctx) : IO Unit := do
  ...
  let json ←
    parseJson (← fmt ctx #["format", "-", "--stdin-filename", ctx.identity, "--json"] pair).stdout
        "pair json"
  let formatted := (json.getObjValAs? String "formatted").toOption.getD ""
  let stop :=
    ((jsonAt? json [.field "sourceMap", .index 1, .field "output", .field "stop"]) |>.bind
            (·.getNum?.toOption)).getD
        0 |>.mantissa.toNat
  ...
  let ranged ←
    expectExit 0 "nonl range" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range", "8:23"] (input? :=
        some (← IO.FS.readFile nonl)) (cwd? := some ctx.root)
  ensureEq "a range over the last unit preserves a missing final newline" "module\n\ndef x :=\n  1"
      ranged.stdout
...
private def testCustomSyntaxAndTail (ctx : Ctx) : IO Unit := do
  ...
  let ranged ←
    expectExit 0 "syn range" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range-lines", "5:1-5:20"] (input? :=
        some (← IO.FS.readFile syn)) (cwd? := some ctx.root)
  ...
  let whole ←
    expectExit 0 "exit stream" ctx.app #["format", "-", "--stdin-filename", ctx.identity] (input? :=
        some (← IO.FS.readFile terminal)) (cwd? := some ctx.root)
...
private def testHeaderEmptyNested (ctx : Ctx) (dirty : System.FilePath) : IO Unit := do
  ...
  let headerRanged ←
    expectExit 0 "header range" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range", "0:20"] (input? :=
        some (← IO.FS.readFile hdr)) (cwd? := some ctx.root)
  ...
  ensureEq "an empty range selects the unit containing it" s!"{ctx.identity}: formatted range 30-51"
      (← actual ctx #["--range", "30:30"] dirty)
...
private def testSuppressionAndOwnership (ctx : Ctx) : IO Unit := do
  ...
  let suppressedRange ←
    expectExit 0 "suppressed range" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range-lines", "3:1-4:38"] (input? :=
        some (← IO.FS.readFile suppressed)) (cwd? := some ctx.root)
  ...
  let resumedRange ←
    expectExit 0 "resumed range" ctx.app
        #["format", "-", "--stdin-filename", ctx.identity, "--range-lines", "6:1-6:39"] (input? :=
        some (← IO.FS.readFile suppressed)) (cwd? := some ctx.root)
  ...
  let some headerIndex :=
    resumedLines.findIdx?
      (·.contains "def resumed") | throw <| IO.userError "the resumed header is missing"
...
private def testPipes (ctx : Ctx) : IO Unit := do
  ...
  writeFile nest
      "module\n\nstructure Point where\n  x : Nat\n  y : Nat\n\ndef  origin   : Point   :=   { x := 0, y := 0 }\n"
  let once ←
    expectExit 0 "once" ctx.app #["format", "-", "--stdin-filename", ctx.identity] (input? :=
        some (← IO.FS.readFile nest)) (cwd? := some ctx.root)
  let twice ←
    expectExit 0 "twice" ctx.app #["format", "-", "--stdin-filename", ctx.identity] (input? :=
        some once.stdout) (cwd? := some ctx.root)
  ...
  let pipe ←
    runProc "bash" #[pipeScript.toString, ctx.app, ctx.identity, big.toString, errPath.toString]
        (cwd? := some ctx.root) (timeoutMs := some 600000)
  ensureEq "a stdout that goes away is not exit 2" "2" (pipe.stdout.trimAscii.toString)
  ensure ((← IO.FS.readFile errPath).contains "broken pipe") "  ... and does not say so on stderr"
  ...
  writeFile decodeScript
     ...
     printf 'EXIT:%s' \"$?\"\n"
  let decoded ←
    runProc "sh" #[decodeScript.toString, ctx.app, ctx.identity, binary.toString] (cwd? :=
        some ctx.root)
        ...
        (try
            runCases "stream"
                #[{ name := "usage-rejections", run := Stream.testUsageRejections ctx dirty },
                  ...
                  { name := "suppression-and-ownership",
                    run := Stream.testSuppressionAndOwnership ctx },
