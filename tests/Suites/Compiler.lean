...
private def lakeEnv : Array (String × Option String) := #[("LEAN_NUM_THREADS", some "1")]

private def lakeBuild (ctx : Ctx) (args : Array String) (label : String)
    (env : Array (String × Option String) := #[]) : IO Unit := do
  discard <| expectExit 0 label "lake" (#["build"] ++ args) (cwd? := some ctx.root)
    (env := lakeEnv ++ env)
...
private def lakeFacetBuildMayFail (ctx : Ctx) : IO UInt32 := do
  let result ← runProc "lake" #["build", "+Broken:leanFmtArtifact"] (cwd? := some ctx.root)
    (env := lakeEnv)
...
private def traceHash (ctx : Ctx) : IO Lean.Json := do
  ...
  let some hash := jsonAt? parsed [.field "depHash"]
    | throw <| IO.userError "LocalSyntax.trace carries no depHash"
  return hash
...
private unsafe def testFacetBuildAndVerify (ctx : Ctx) : IO Unit := do
  ...
  discard <| expectExit 0 "cold facet build" "lake" #["-R", "build", "+LocalSyntax:leanFmtArtifact"]
...
private def lakeQuery (ctx : Ctx) (target : String) : IO String := do
  let result ← expectExit 0 s!"lake query {target}" "lake" #["-q", "query", target, "--text"]
    (cwd? := some ctx.root)
...
private unsafe def testExtractorExactOlean (ctx : Ctx) : IO Unit := do
  ...
  let leanBinResult ← expectExit 0 "lake env which lean" "lake" #["env", "which", "lean"]
    (cwd? := some ctx.root)
  ...
  withScratchDir "compiler-shadow" fun shadow => do
    ...
    discard <| expectExit 0 "shadow compile" leanBin
      #[s!"--plugin={plugin}", "-o", "LocalSyntax.olean", "LocalSyntax.lean"]
      (cwd? := some shadow) (env := #[("LEAN_PATH", some repoLib)])
    let exactJson := shadow / "exact.json"
    discard <| expectExit 0 "extractor" extractor
      #["LocalSyntax", ctx.olean.toString, exactJson.toString]
      (env := #[("LEAN_PATH", some s!"{shadow}:{repoLib}")])
...
private unsafe def testRuleEditNoInvalidation (ctx : Ctx) : IO Lean.Json := do
  discard <| expectExit 0 "facet rebuild" "lake" #["-R", "build", "+LocalSyntax:leanFmtArtifact"]
    (cwd? := some ctx.root) (env := lakeEnv)
  ...
  writeFile ctx.rulesSource (rules.replace
    "message := \"redundant nested parentheses\"" "message := \"probe: redundant nested parentheses\"")
  lakeFacetBuild ctx
  ...
  ensureEq "editing a rule changed the compiled bytes of an unrelated module"
    enabledOlean (← sha256 ctx.olean)
  verifyArtifacts ctx
...
private unsafe def testPluginEditInvalidation (ctx : Ctx) (enabledTrace : Lean.Json) : IO Unit := do
  ...
  writeFile ctx.pluginSource (plugin.replace "private def produceCommandRecord"
...
private unsafe def testLakeCacheRestore (ctx : Ctx) : IO Unit := do
  ...
  withScratchDir "compiler-lake-cache" fun cacheDir => do
    ...
    removeFile? ctx.artifact
    removeFile? (ctx.artifact.toString ++ ".hash")
    let restored ← expectExit 0 "cache-restored facet build" "lake"
      #["-v", "build", "+LocalSyntax:leanFmtArtifact"] (cwd? := some ctx.root) (env := lakeEnv ++ env)
...
private unsafe def cases (ctx : Ctx) : Array Case := Id.run do
  let mut all := earlyCases ctx
  all := all.push { name := "rule-edit-boundary", run := do
...
public unsafe def main (args : List String) : IO UInt32 := do
  ...
  let cleanup : IO Unit := do
    ...
    try CompilerSuite.lakeFacetBuild ctx "post-suite facet restore" catch _ => pure ()
  let code ← try
