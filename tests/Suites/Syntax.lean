...
private def expectCodes (ctx : Ctx) (label fixture : String) (expected : List String)
  ...
  let codes := (((jsonAt? files[0]! [.field "findings"]).bind (·.getArr?.toOption)).getD #[]).map
...
private def testFixSpans (ctx : Ctx) : IO Unit := do
  let fixOf (label fixture selector : String) : IO Lean.Json := do
    ...
    let some finding := (jsonAt? report [.field "files", .index 0, .field "findings", .index 0])
      | throw <| IO.userError s!"{label}: no finding"
    let some fix := jsonAt? finding [.field "fix"] | throw <| IO.userError s!"{label}: no fix"
    return fix
  let pinEdits (fix : Lean.Json) (expected : List (Nat × Nat × String)) (label : String) : IO Unit := do
    ...
    let some edits := (jsonAt? fix [.field "edits"]).bind (·.getArr?.toOption)
      | throw <| IO.userError s!"{label}: fix has no edits"
    let got := edits.toList.map fun edit =>
  ...
  pinEdits (← fixOf "fmt013-pos" "NestedParen.lean" "FMT011") [(51, 52, ""), (55, 56, "")] "fmt013-pos"
...
private def testNegatives (ctx : Ctx) : IO Unit := do
  for (label, fixture) in [
    ("clean", "Clean.lean"),
    ("near-008", "NearNoDecl.lean"),
    ("near-009", "NearOpenSection.lean"),
    ("near-009-dotted", "EndDotted.lean"),
    ("near-009-nested", "ScopesBalanced.lean"),
    ("near-010", "NearAttr.lean"),
    ("near-011", "NearDeriving.lean"),
    ("near-012", "NearOption.lean"),
    ("near-013", "NearParen.lean"),
    ("comment", "Comment.lean"),
    ("quote-paren", "QuoteParen.lean"),
    ("quote-attr", "QuoteAttr.lean"),
    ("custom-syntax", "CustomSyntax.lean")
  ] do
...
private def testMalformed (ctx : Ctx) : IO Unit := do
  ...
  ensure (((jsonAt? report [.field "files", .index 0, .field "status"]).bind (·.getStr?.toOption)) ==
      some "broken")
...
private def fixApplies (ctx : Ctx) (label fixture selector gone present : String) : IO Unit := do
  ...
  ensureJsonAt report [.field "files", .index 0, .field "written"] (Lean.toJson true) s!"{label}-fix"
  ...
  let recheck ← expectExit 0 s!"{label}-recheck" ctx.application
    #["check", "--root", ".", "--json", "--no-cache", "--preview", "--select", selector,
      probe.toString]
    (cwd? := some ctx.root) (env := sfmtEnv)
...
private def testFormatNeverFixes (ctx : Ctx) : IO Unit := do
  ...
  let some file := jsonAt? report [.field "files", .index 0]
    | throw <| IO.userError "fmt013-format: no file"
  let codes := (((jsonAt? file [.field "findings"]).bind (·.getArr?.toOption)).getD #[]).map
    fun finding => (finding.getObjValAs? String "code").toOption.getD ""
...
private def testMultiRuleComposition (ctx : Ctx) : IO Unit := do
  ...
  let refix ← expectExit 0 "mover-refix" ctx.application
    (#["fix", "--root", ".", "--json", "--no-cache", "--preview"] ++ selectors ++ #[probe.toString])
    (cwd? := some ctx.root) (env := sfmtEnv)
  ...
  ensure (((jsonAt? refixed [.field "files", .index 0, .field "status"]).bind (·.getStr?.toOption)) ==
      some "clean")
...
private def testPassOrderIndependence (ctx : Ctx) : IO Unit := do
  ...
  discard <| expectExit 0 "order-a" ctx.application
    ...
    (cwd? := some ctx.root) (env := sfmtEnv)
  discard <| expectExit 0 "order-b" ctx.application
