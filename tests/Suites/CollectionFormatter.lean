...
private def testWidth (root setup : System.FilePath) (application : String) (width : Nat) :
  ...
  | _ => throw <| IO.userError s!"width {width}: match arms missing"
  ...
  | _, _ => throw <| IO.userError s!"width {width}: associativity declarations missing"
...
private def testNarrowLayout (root setup : System.FilePath) (application : String) : IO Unit := do
  ...
  ensureFlex "width 20" narrow
    [.lit "{", .anyWs, .lit "first", .anyWs, .lit ":=", .anyWs, .lit "alpha,",
     .anyWs, .lit "second", .anyWs, .lit ":=", .anyWs, .lit "beta,", .anyWs,
     .lit "third", .anyWs, .lit ":=", .anyWs, .lit "gamma", .anyWs, .lit "}"]
  ensureFlex "width 20" narrow
    [.lit "{", .anyWs, .lit "packet", .someWs, .lit "with", .newline,
     .lit "first", .anyWs, .lit ":=", .anyWs, .lit "alpha,", .anyWs,
     .lit "second", .anyWs, .lit ":=", .anyWs, .lit "beta", .anyWs, .lit "}"]
  ensureFlex "width 20" narrow
    [.lit "{", .anyWs, .lit "first", .anyWs, .lit ":=", .anyWs, .lit "alpha", .someWs,
     .lit "second", .anyWs, .lit ":=", .anyWs, .lit "beta", .someWs,
     .lit "third", .anyWs, .lit ":=", .anyWs, .lit "gamma", .anyWs, .lit "}"]
  ensureFlex "width 20" narrow
...
private def testWideLayout (root setup : System.FilePath) (application : String) : IO Unit := do
  ...
  for flat in ["(alpha, beta, gamma, delta)", "[alpha, beta, gamma, delta, epsilon]",
...
public def main (args : List String) : IO UInt32 := do
  ...
  withScratchDir "collection-formatter" fun work => do
    ...
    let cases : Array Case := #[
      ...
      { name := "narrow-layout", run := CollectionFormatter.testNarrowLayout root setup application },
      { name := "wide-layout", run := CollectionFormatter.testWideLayout root setup application }
    ]
