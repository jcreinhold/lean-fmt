...
private def exotic (ctx : Ctx) (name content : String) : IO Lean.Json := do
  ...
  let envelope ← project ctx ctx.borrowedSetup.toString source.toString s!"{name}.lean"
    s!"exotic:{name}"
...
private def testExotic (ctx : Ctx) : IO Unit := do
  ...
  discard <| exotic ctx "eofcomments"
  ...
  let choiceEnvelope ← exotic ctx "choice"
    "module\n\nstructure P where\n  a : Nat\n  b : Nat\n\ndef mk (a b : Nat) : P := { a, b }\n"
  let some kinds := ((((jsonAt? choiceEnvelope [.field "artifact", .field "syntaxData",
      .field "kinds"]).bind (·.getArr?.toOption)).getD #[]).mapM (·.getStr?.toOption))
...
private def fail {α} (message : String) : IO α := throw <| IO.userError message
...
private def mapCommands (projection : Lean.Json) (edit : Array Lean.Json → Array Lean.Json) :
    Lean.Json :=
  let commands := (projection.getObjVal? "commands").toOption |>.bind (·.getArr?.toOption)
    |>.getD #[]
...
private def shiftInfoField (entry : Lean.Json) (field : Nat) (delta : Int) : Lean.Json :=
  ...
  | .arr items =>
    ...
    | some rawInfo =>
      match rawInfo with
      | .arr info =>
        let shifted := info.modify field fun value =>
          match value with
...
private def leafPositions (entries : Array Lean.Json) : IO (Array Nat) := do
  ...
  for index in [0:entries.size] do
    ...
    | .arr items =>
      let tag := items[0]? |>.bind fun value =>
        match value with | .num n => some n.mantissa | _ => none
      if tag == some 2 || tag == some 3 then
        if items[1]?.map (fun info => match info with | .arr _ => true | _ => false)
            |>.getD false then
  ...
  for index in ← leafPositions entries do
    ...
    | .arr items =>
      ...
      | some rawInfo =>
        match rawInfo with
        | .arr info =>
          let stop := info[4]? |>.bind fun value =>
            match value with | .num n => some n.mantissa | _ => none
          let endPos := info[3]? |>.bind fun value =>
            match value with | .num n => some n.mantissa | _ => none
...
private partial def subtreeLeaves (entries : Array Lean.Json) (index : Nat) :
  ...
  | .arr items =>
    let tag := items[0]? |>.bind fun value =>
      match value with | .num n => some n.mantissa.toNat | _ => none
    match tag with
    | some 0 => return (index + 1, #[])
    | some 1 =>
      let some childCount := items[3]? |>.bind fun value =>
          match value with | .num n => some n.mantissa.toNat | _ => none
      ...
      for _ in [0:childCount] do
        ...
        found := found ++ child
      return (cursor, found)
    | some _ => return (index + 1, #[index])
    | none => fail s!"entry {index} has no tag"
  | _ => fail s!"entry {index} is malformed"
  ...
  for index in [0:entries.size] do
    ...
    | .arr items =>
      let tag := items[0]? |>.bind fun value =>
        match value with | .num n => some n.mantissa | _ => none
      if tag == some 1 then
        let kindIndex := items[2]? |>.bind fun value =>
          match value with | .num n => some n.mantissa.toNat | _ => none
...
private def expectRejection (name : String) (check : IO LeanFmt.Test.Projection.Measurements) :
  ...
  match outcome with
  | none => fail s!"the oracle accepted a {name} mutation; it has no teeth"
  | some message => return s!"{name}: {message}"
...
private def testMutations (ctx : Ctx) : IO Unit := do
  ...
  let baseEnvelope ← project ctx ctx.borrowedSetup.toString unicodeSource.toString "unicode.lean"
    "mutate-base"
  let some base := jsonAt? baseEnvelope [.field "artifact"]
    | fail "mutation base has no artifact"
  let choiceSource := ctx.work / "choice.lean"
  let choiceEnvelope ← project ctx ctx.borrowedSetup.toString choiceSource.toString "choice.lean"
    "mutate-choice"
  ...
  let checks : Array (String × (Lean.Json → IO Lean.Json)) := #[
    ...
    ("wrong digest", fun artifact => pure <|
      artifact.setObjVal! "normalizedDigest" (Lean.toJson (String.ofList (List.replicate 64 '0')))),
    ("wrong length", fun artifact => do
      let bytes ← jsonNat ((artifact.getObjVal? "normalizedBytes").toOption.getD .null)
        "normalizedBytes"
      pure <| artifact.setObjVal! "normalizedBytes" (Lean.toJson (bytes + 1))),
    ("stale schema", fun artifact => pure <|
      artifact.setObjVal! "schema" (Lean.toJson "lean-fmt.module-artifact.v0")),
    ("carries findings", fun artifact => pure <|
      artifact.setObjVal! "findings" (.arr #[])),
    ...
    ("command range past end", fun artifact => pure <| mapSyntax artifact fun projection =>
      mapCommands projection (·.modify 0 fun command =>
        ...
        | some range =>
          let stop := ((range.getObjVal? "stop").toOption |>.bind fun value =>
            match value with | .num n => some n.mantissa.toNat | _ => none).getD 0
          command.setObjVal! "range" (range.setObjVal! "stop" (Lean.toJson (stop + 1000000000)))
        | none => command)),
    ("terminal misplaced", fun artifact =>
      let projection := syntaxOf artifact
      let terminal := ((projection.getObjVal? "terminal").toOption |>.bind fun value =>
        match value with | .num n => some n.mantissa.toNat | _ => none).getD 0
      pure <| mapSyntax artifact (·.setObjVal! "terminal" (Lean.toJson (terminal + 1)))),
    ("truncated entries", fun artifact => pure <| mapSyntax artifact fun projection =>
      setEntries projection (entriesOf projection).pop),
    ("trailing entry", fun artifact => pure <| mapSyntax artifact fun projection =>
    ...
    ("dropped leaf", fun artifact => do
      ...
      pure <| mapSyntax artifact fun projection =>
      ...
      | .arr items =>
        ...
        | some rawInfo =>
          ...
          | .arr info =>
            ...
            let synthetic : Lean.Json := .arr
              #[Lean.toJson (2 : Nat), positionField, endField, Lean.toJson true]
            pure <| mapSyntax artifact fun projection =>
              setEntries projection (entries.set! position (.arr (items.set! 1 synthetic)))
          | _ => fail "synthetic leaf: no info"
        | none => fail "synthetic leaf: no info"
      | _ => fail "synthetic leaf: malformed entry"),
    -- Dangling references.
    ("bad kind ref", fun artifact =>
      let projection := syntaxOf artifact
      let entries := entriesOf projection
      pure <| mapSyntax artifact fun x =>
        setEntries x (entries.modify 0 fun entry =>
          match entry with
  ...
  for (name, edit) in checks do
    ...
    let message ← expectRejection name <|
      LeanFmt.Test.Projection.checkArtifact mutated unicodeSource
  ...
  let disagreement := mapSyntax choiceBase fun projection =>
    setEntries projection (choiceEntries.modify firstLeaf (shiftInfoField · 4 1))
  let message ← expectRejection "choice disagreement" <|
...
public def main (args : List String) : IO UInt32 := do
  ...
  withScratchDir "lossless" fun work => do
    ...
    let corpus := (modules ++ extras).map fun moduleName =>
