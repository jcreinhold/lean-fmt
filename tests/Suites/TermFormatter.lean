...
private def expectedValidation : Lean.Json :=
  Lean.Json.mkObj
...
private def spellings : Array FlexPattern :=
  #[[.lit "alpha", .anyWs, .lit "+", .anyWs, .lit "beta"], [.lit "Nat.succ", .someWs, .lit "value"],
    [.lit "pair.1", .anyWs, .lit "+", .anyWs, .lit "pair.2"], [.lit "⊕custom"],
    [.lit "(value", .anyWs, .lit ":", .anyWs, .lit "Nat)"], [.lit "@Nat.succ"]]
...
private def widthText (root setup : System.FilePath) (application : String) (width : Nat) :
    IO String := do
  let report ←
    analyzeExact root application setup "tests/fixtures/term-formatter/Terms.lean" "Terms.lean"
        s!"4:{width}"
...
private def testWidth20 (root setup : System.FilePath) (application : String) : IO Unit := do
  ...
  match
    (["| 0, _ => alpha", "| _, 1 => beta"].map fun needle =>
  ...
  | _ =>
    throw <| IO.userError "impossible: splitOn returns a head"
...
private def testWidthDistinctness (root setup : System.FilePath) (application : String) : IO Unit :=
  do
