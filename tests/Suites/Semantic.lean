...
private def artifactOf (envelope : Lean.Json) (label : String) : IO Lean.Json := do
  let some artifact := jsonAt? envelope [.field "artifact"]
    | throw <| IO.userError s!"{label}: envelope has no artifact"
  return artifact
...
private def ensureNull (json : Lean.Json) (path : List JsonStep) (label : String) : IO Unit := do
  ...
  | none => pure ()
  | some .null => pure ()
  | some other => throw <| IO.userError s!"{label}: {other.compress}"
...
private def testCaptureGating (ctx : Ctx) : IO Unit := do
  ...
  let semanticKeys := (((jsonAt? on [.field "semantic"]).bind (·.getObj?.toOption)).map
...
private def testDiagnosticsDifferential (ctx : Ctx) : IO Unit := do
  ...
  let captured := (((jsonAt? on [.field "semantic", .field "diagnostics"]).bind
  ...
  let want := ["Lean.Linter.deprecatedAttr", "linter.unusedVariables", "linter.unusedSectionVars",
  ...
  for object in oracle do
    ...
    if !want.contains kind then continue
...
private def testOccurrencesDifferential (ctx : Ctx) : IO Unit := do
  ...
  let some occ := (jsonAt? occurrences [.field "semantic", .field "occurrences"]).bind
      (·.getArr?.toOption)
    | throw <| IO.userError "token 2 captured no occurrences"
  ...
  let uses := occ.toList.filter fun o =>
    (((o.getObjValAs? String "declName").toOption.getD "").splitOn ".").getLast?.getD "" == "oldName"
  let some use := uses.head?
    | throw <| IO.userError s!"expected exactly one oldName use, got {uses.length}"
  ...
  let starts := oracle.toList.filter (fun o =>
      (o.getObjValAs? String "kind").toOption == some "Lean.Linter.deprecatedAttr") |>.map
    fun o => byteOffset source (natAt? o [.field "pos", .field "line"] |>.getD 0)
...
private def testFixablePredicate (ctx : Ctx) : IO Unit := do
  ...
  let source ← IO.FS.readFile (ctx.root / fixture)
  let some occ := (jsonAt? artifact [.field "semantic", .field "occurrences"]).bind
      (·.getArr?.toOption)
    | throw <| IO.userError "no occurrences captured"
  ...
  let checkOne (spelled : String) (wantFixable : Bool) (wantDecl : String) : IO Unit := do
    let some o := occ.toList.find? (fun entry => spelledOf entry == spelled)
      | throw <| IO.userError s!"no occurrence spelled {spelled}"
    let fixable := ((o.getObjValAs? Bool "fixable").toOption).getD false
    ensure (fixable == wantFixable) s!"occurrence {spelled} fixable={fixable}, expected {wantFixable}"
...
private def testAcceptance (ctx : Ctx) : IO Unit := do
  ...
  discard <| expectExit 0 "lake build Demo" "lake" #["-d", proj.toString, "build", "Demo"]
    (cwd? := some ctx.root)
  ...
  let broken ← check #["check", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--select", "FMT012", (proj / "acc" / "Broken.lean").toString] 1 "acc broken"
  ...
  let mixedReport ← check #["check", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--select", "FMT011", "--select", "FMT012", mixed.toString] 1 "acc mixed"
  let mixedFindings := ((jsonAt? mixedReport [.field "files", .index 0, .field "findings"]).bind
    (·.getArr?.toOption)).getD #[]
  ...
  let some dep := mixedFindings.toList.find? (fun f =>
      (f.getObjValAs? String "code").toOption == some "FMT012")
    | throw <| IO.userError "mixed-tier: no FMT012 finding"
  ...
  let withheld ← check #["fix", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--select", "FMT012", mixed.toString] 0 "acc withheld"
  ...
  let applied ← check #["fix", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--unsafe-fixes", "--select", "FMT012", mixed.toString] 0 "acc apply"
  ...
  let recheck ← check #["check", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--select", "FMT012", mixed.toString] 0 "acc recheck"
  let recheckCodes := (((jsonAt? recheck [.field "files", .index 0, .field "findings"]).bind
    (·.getArr?.toOption)).getD #[]).toList.map
  ...
  let formatReport ← check #["format", "--check", "--root", proj.toString, "--json", "--no-cache",
    "--preview", "--unsafe-fixes", "--select", "FMT012", mixedFmt.toString] 1 "acc format"
  match (jsonAt? formatReport [.field "files", .index 0, .field "formatted"]).bind
  ...
  let idem ← check #["fix", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--unsafe-fixes", "--select", "FMT012", mixed.toString] 0 "acc idem"
  ...
  discard <| check #["fix", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--unsafe-fixes", "--select", "FMT012", "--select", "FMT011", orderA.toString] 0 "order A"
  discard <| check #["fix", "--root", proj.toString, "--json", "--no-cache", "--preview",
    "--unsafe-fixes", "--select", "FMT011", "--select", "FMT012", orderB.toString] 0 "order B"
...
private def testCost (ctx : Ctx) : IO Unit := do
  ...
  let rssOf (stats : String) (label : String) : IO Nat := do
    for line in stats.splitOn "\n" do
      if line.contains "maximum resident set size" then
        ...
        | some n => return n
        | none => throw <| IO.userError s!"{label}: unparsable RSS line {line}"
      ...
      if line.contains " real" then
        ...
        | some word =>
          ...
          | [whole, frac] =>
            ...
            | _, _ => throw <| IO.userError s!"{label}: unparsable real line {line}"
          | [whole] =>
            match whole.toNat? with
            | some w => return w.toFloat
            | none => throw <| IO.userError s!"{label}: unparsable real line {line}"
          | _ => throw <| IO.userError s!"{label}: unparsable real line {line}"
        | none => throw <| IO.userError s!"{label}: unparsable real line {line}"
  ...
  IO.println s!"   cost: RSS diag-capture {onRss / 1048576} MiB, occ-capture \
...
public def main (args : List String) : IO UInt32 := do
  ...
  discard <| expectExit 0 "lake build lean-fmt Clean:leanFmtArtifact" "lake"
