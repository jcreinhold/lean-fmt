...
private def Ctx.app (ctx : Ctx) : String := ctx.application
private def Ctx.rootStr (ctx : Ctx) : String := ctx.root.toString
...
private def requestFrame (id : Nat) (method : String) (params : Option Json := none) : String :=
  ...
  frame (Json.mkObj (match params with
...
private def openFrame (uri text : String) (version : Nat := 1) : String :=
  notificationFrame "textDocument/didOpen" (some (Json.mkObj [("textDocument", Json.mkObj [
...
private partial def parseFrames (bytes : ByteArray) (label : String) (index : Nat := 0)
  ...
  if index >= bytes.size then return acc
  let mut headerStop := index
  let mut found := false
  for i in [index:bytes.size - 3] do
    if !found && bytes[i]! == 0x0D && bytes[i + 1]! == 0x0A &&
        bytes[i + 2]! == 0x0D && bytes[i + 3]! == 0x0A then
      headerStop := i
      found := true
  unless found do
  ...
  let some length := length?
    | throw <| IO.userError s!"{label}: no Content-Length in {header}"
  ...
  let json ← match Json.parse (String.fromUTF8! body) with
    | .ok value => pure value
    | .error message => throw <| IO.userError s!"{label}: a frame that is not JSON: {message}"
...
private def runSession (ctx : Ctx) (payload : String) (label : String) :
    IO (UInt32 × Array Json) := do
  let result ← runProc ctx.app #["lsp", "--root", ctx.rootStr] (input? := some payload)
    (cwd? := some ctx.root) (env := #[("LEAN_NUM_THREADS", some "1")])
...
private def answerOf (messages : Array Json) (id : Nat) : IO Json := do
  let some (_, answer) := (responsesOf messages).find? (·.1 == id)
    | throw <| IO.userError s!"no response to id {id} in the stream"
  return answer

private def notificationsOf (messages : Array Json) (method : String) : Array Json :=
  messages.filter fun message =>
    (message.getObjValAs? String "method").toOption == some method
...
private def testLifecycle (ctx : Ctx) : IO Unit := do
  ...
  ensureJsonAt (← answerOf messages 1)
    [.field "result", .field "capabilities", .field "codeActionProvider", .field "codeActionKinds"]
    (Lean.toJson #["quickfix", "source.fixAll", "source.organizeImports"]) "code action kinds"
  ensureJsonAt (← answerOf messages 1)
    [.field "result", .field "serverInfo", .field "name"] (Lean.toJson "lean-fmt")
...
private def testInitializationGuard (ctx : Ctx) : IO Unit := do
  ...
  let (_, messages) ← runSession ctx
    (requestFrame 1 "$/lean-fmt/health"
      ++ requestFrame 2 "initialize" (some (Json.mkObj []))
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "pre-initialize"
  ensureJsonAt (← answerOf messages 1) [.field "error", .field "code"]
    (Lean.toJson (-32002 : Int)) "a request before initialize is refused"
  ensure (((answerOf' messages 2).getObjVal? "result").toOption.isSome) "initialize still works afterwards"
  -- `exit` without `shutdown` is a protocol violation and exits non-zero.
  let (code, _) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj [])) ++ notificationFrame "exit") "bare exit"
...
private def testMalformedRecovery (ctx : Ctx) : IO Unit := do
  let (code, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ...
      ++ requestFrame 2 "$/lean-fmt/health"
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "malformed"
  let parseErrors := messages.filter fun message =>
    ((jsonAt? message [.field "error", .field "code"]).bind (·.getNum?.toOption)).map
      (·.mantissa == -32700) |>.getD false
...
private def testUnknownMethods (ctx : Ctx) : IO Unit := do
  let (code, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ...
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "unknown"
  ensureJsonAt (← answerOf messages 2) [.field "error", .field "code"]
    (Lean.toJson (-32601 : Int)) "an unknown request is MethodNotFound"
...
private def testDocumentAdmission (ctx : Ctx) : IO Unit := do
  ...
  let (_, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ++ openFrame (findingsUri ctx) source
      ...
      ++ requestFrame 2 "$/lean-fmt/health"
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "admission"
  ...
  ensure (shown.any fun message => message.contains "no file location" && message.contains "untitled:Untitled-1") "an untitled buffer is refused for having no location"
  ensure (shown.any (·.contains "outside the project root")) "a document outside the root is refused"
  ensure (shown.any (·.contains "Lake build directory")) "a document inside .lake is refused"
  ensure (shown.any (·.contains "not a Lean source")) "a document that is not Lean source is refused"
  ensure (shown.all fun message => message.contains "file://" || message.contains "untitled:") "every refusal names the URI the client sent"
  -- The refusal is announced once — not per request against the refused document.
  let (_, onceMessages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ++ openFrame "untitled:Untitled-1" "def x := 1\n"
      ++ requestFrame 2 "shutdown"
      ++ notificationFrame "exit") "refusal-once"
...
private def testVersionsSync (ctx : Ctx) : IO Unit := do
  ...
  let (_, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ++ openFrame uri "def a := 1\n"
      -- An incremental change: replace `1` with `22`.
      ++ notificationFrame "textDocument/didChange" (some (Json.mkObj [
        ("textDocument", Json.mkObj [("uri", Json.str uri), ("version", Lean.toJson (2 : Nat))]),
        ("contentChanges", Json.arr #[Json.mkObj [
      ...
      ++ requestFrame 2 "$/lean-fmt/health"
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "versions"
  ...
  ensure ((loggedMessages messages).any (·.contains "not newer than")) "a non-increasing version is refused and said so"
...
private def testCloseClears (ctx : Ctx) : IO Unit := do
  ...
  let (_, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ++ openFrame uri source
      ++ notificationFrame "textDocument/didClose" (some (Json.mkObj [
        ("textDocument", Json.mkObj [("uri", Json.str uri)])]))
      ++ requestFrame 2 "$/lean-fmt/health"
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "close"
  ...
  ensureJsonAt published[0]! [.field "params", .field "diagnostics"] (.arr #[])
    "and it is empty"
...
private def testCancellation (ctx : Ctx) : IO Unit := do
  ...
  let (_, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ...
      ++ notificationFrame "exit") "cancellation"
  ensureJsonAt (← answerOf messages 2) [.field "error", .field "code"]
    (Lean.toJson (-32800 : Int)) "a cancelled request is answered RequestCancelled"
  ensureEq "and it is answered exactly once" 1
    (messages.filter fun message =>
      (message.getObjValAs? Nat "id").toOption == some 2).size
  ensure (((field (← answerOf messages 3) "result")) != .null) "an uncancelled request is unaffected"
...
private def testBounds (ctx : Ctx) : IO Unit := do
  ...
  let (code, messages) ← runSession ctx
    (requestFrame 1 "initialize" (some (Json.mkObj []))
      ++ openFrame (findingsUri ctx) big
      ++ requestFrame 2 "$/lean-fmt/health"
      ++ requestFrame 3 "shutdown"
      ++ notificationFrame "exit") "bounds"
  ensureJsonAt (← answerOf messages 2) [.field "result", .field "openDocuments"]
...
private def liveSession {α : Type} (ctx : Ctx) (body : IpcM α) (debounceMs : Nat := 1)
  ...
  LspClient.withServer ctx.app ctx.rootStr (do
...
private def resultOf (answer : Answer) (label : String) : IO Json := do
  ...
  | .error code message => throw <| IO.userError s!"{label}: {(Lean.toJson code).compress} {message}"

private def editsOf (answer : Answer) (label : String) : IO (Array Json) := do
  let value ← resultOf answer label
  match value with
  | .arr edits => return edits
  | _ => throw <| IO.userError s!"{label}: the result is not an edit array: {value.compress}"
...
private def testFormatting (ctx : Ctx) : IO Unit := do
  ...
  let cleanSource ← readFixture ctx "Clean.lean"
  let ((narrowEdit, wholeText), code) ← liveSession ctx do
    openDocument (cleanUri ctx) cleanSource
    request 1 "textDocument/formatting" (LspClient.documentParam (cleanUri ctx))
    ensureJsonAt (← resultOf (← awaitResponse 1) "clean formatting") []
      (.arr #[]) "a canonical document needs no edits"
    ...
    ensure (field (field narrowEdit "range") "end" != LspClient.position 2 5) "the actual range is not the requested one"
    ensure (!(strField narrowEdit "newText").contains "def layoutValue") "and the replacement is only the selected unit"
...
private def testCodeActions (ctx : Ctx) : IO Unit := do
  ...
  let ((), code) ← liveSession ctx do
    ...
    let some quickfix := actions.find? (strField · "kind" == "quickfix")
      | throw <| IO.userError "no quickfix among the actions"
    ...
    ensureJsonAt changes[0]! [.field "textDocument", .field "uri"] (Lean.toJson (findingsUri ctx))
      "for the document the action was asked about"
    ensureJsonAt changes[0]! [.field "edits", .index 0, .field "newText"] (Lean.toJson "")
      "and it deletes rather than rewrites"
    let some fixAll := actions.find? (strField · "kind" == "source.fixAll")
      | throw <| IO.userError "no fix-all among the actions"
    ensureJsonAt fixAll [.field "edit", .field "documentChanges", .index 0, .field "edits",
      .index 0, .field "range", .field "start"] (LspClient.position 0 0)
      "fix-all rewrites the whole document"
...
private def testIgnoreOption (ctx : Ctx) : IO Unit := do
  ...
  -- dropped.
  let findingsSource ← readFixture ctx "Findings.lean"
  let ((), code) ← liveSession ctx (options := Json.mkObj [("ignore", Lean.toJson #["FMT003"])]) do
    openDocument (findingsUri ctx) findingsSource
    let some diagnostics ← awaitDiagnostics (findingsUri ctx)
...
private def testUnsafeDemoted (ctx : Ctx) : IO Unit := do
  ...
  let quickfixes (extraArgs : Array String) : IO (Nat × List String × UInt32) := do
    let ((reported, kinds), code) ← liveSession ctx
      (extraArgs := #["--config", ctx.configToml.toString] ++ extraArgs) do
      openDocument (findingsUri ctx) findingsSource
      let some diagnostics ← awaitDiagnostics (findingsUri ctx)
...
private partial def awaitPublication (uri : String) (version : Nat) (budget : Nat := 64) :
    IpcM Json := do
  if budget == 0 then throw <| IO.userError s!"no publication for version {version} arrived"
  let json ← readFrameJson
  let params := field json "params"
  if (json.getObjValAs? String "method").toOption == some "textDocument/publishDiagnostics"
...
private def testSuperseded (ctx : Ctx) : IO Unit := do
  ...
  -- edited past.
  let layoutSource ← readFixture ctx "Layout.lean"
  let ((), code) ← liveSession ctx (debounceMs := 80) do
    openDocument (layoutUri ctx) layoutSource
    for version in [2, 3, 4] do
      notify "textDocument/didChange" (Json.mkObj [
        ...
        ("contentChanges", Json.arr #[Json.mkObj [
          ("range", LspClient.range 4 0 4 0), ("text", Json.str s!"-- {version}\n")]])])
