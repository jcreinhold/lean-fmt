...
private def testGithub (ctx : Ctx) : IO Unit := do
  ...
  let hostileOut ← fmt ctx
    #["check", "-", "--stdin-filename", hostile, "--output-format", "github"] (input? := some dup)
...
private def sarifConform (log : Lean.Json) (label : String) : IO Unit := do
  ...
  let some runs := (jsonAt? log [.field "runs"]).bind (·.getArr?.toOption)
    | throw <| IO.userError s!"{label}: no runs"
  ensureEq s!"{label}: one run" 1 runs.size
  let run := runs[0]!
  let some results := (jsonAt? run [.field "results"]).bind (·.getArr?.toOption)
    | throw <| IO.userError s!"{label}: no results"
  ...
  let some region := jsonAt? results[0]!
      [.field "locations", .index 0, .field "physicalLocation", .field "region"]
    | throw <| IO.userError s!"{label}: no region"
  ...
  let described := (((jsonAt? run [.field "tool", .field "driver", .field "rules"]).bind
      (·.getArr?.toOption)).getD #[]).filterMap
  ...
  let some invocations := (jsonAt? run [.field "invocations"]).bind (·.getArr?.toOption)
    | throw <| IO.userError s!"{label}: no invocations"
  ...
  let some srcroot := (jsonAt? run
      [.field "originalUriBaseIds", .field "%SRCROOT%", .field "uri"]).bind (·.getStr?.toOption)
    | throw <| IO.userError s!"{label}: no %SRCROOT%"
...
private def testSarif (ctx : Ctx) : IO Unit := do
  ...
  -- renderer.
  let explained ← expectExit 0 "explain FMT003" ctx.application #["explain", "FMT003", "--json"]
    (cwd? := some ctx.root)
  let summary := ((← parseJson explained.stdout "explain").getObjValAs? String "summary").toOption
  let described := (((jsonAt? (← parseJson report.stdout "sarif")
      [.field "runs", .index 0, .field "tool", .field "driver", .field "rules", .index 0,
        .field "shortDescription", .field "text"])).bind (·.getStr?.toOption))
  ...
  let sabotaged := #[("LEAN_FMT_DISABLE_ARTIFACT", some "1"),
  ...
  let some invocation := jsonAt? failedLog [.field "runs", .index 0, .field "invocations", .index 0]
    | throw <| IO.userError "failed sarif: no invocation"
  ensureJsonAt invocation [.field "executionSuccessful"] (Lean.toJson false) "failed sarif"
  let notifications := ((jsonAt? invocation [.field "toolExecutionNotifications"]).bind
...
private def testJunit (ctx : Ctx) : IO Unit := do
  ...
  writeFile reportPath report.stdout
  discard <| expectExit 0 "the JUnit report is well-formed XML" "xmllint"
  ...
  let parsed ← expectTool "an independent JUnit parser reads it back"
  ...
  let dup := "module\n\nimport LeanFmt.Basic\nimport LeanFmt.Basic\n"
  let xmlHostile ← fmt ctx
    #["check", "-", "--stdin-filename", "a&b<c>d.lean", "--output-format", "junit"]
    (input? := some dup)
  ensureContains xmlHostile.stderr "a&amp;b&lt;c&gt;d.lean"
    "XML metacharacters in a path are escaped"
  discard <| expectExit 0 "  ... and the result is still well-formed" "xmllint"
    #["--noout", "-"] (input? := some xmlHostile.stderr)
...
private def testOutputFiles (ctx : Ctx) : IO Unit := do
  ...
  ensureEq "a directory --output-file is rejected"
    s!"--output-file is a directory: {ctx.work}" (firstStderrLine directory)
  let outSarif := ctx.work / "out.sarif"
  let written ← fmt ctx
    #["check", findings, "--output-file", outSarif.toString, "--output-format", "sarif"]
  ...
  for format in ["text", "concise", "json", "github", "sarif", "junit"] do
    let script :=
      "err=$(\"$APP\" check \"$FINDINGS\" --output-format \"$FMT\" 2>&1 >/dev/null \
        ...
        printf '%s\\n%s' \"$err\" \"$code\""
    let result ← expectExit 0 s!"broken pipe {format}" "bash" #["-c", script]
      (cwd? := some ctx.root)
      (env := #[("APP", some ctx.application), ("FINDINGS", some findings),
        ("FMT", some format)])
...
private def testUris (ctx : Ctx) : IO Unit := do
  ...
  -- its UTF-8 bytes.
  let uriOut ← fmt ctx
    #["check", "-", "--stdin-filename", "src/my dir/Ä#b%c.lean", "--output-format", "sarif"]
    (input? := some dup)
  ...
  let decoded ← expectTool "  ... and an independent URI parser decodes it back to the path"
    #["run", "--quiet", "python3", "-c",
  ...
  ensureEq "  ... and an independent URI parser decodes it back to the path"
    "src/my dir/Ä#b%c.lean" decoded
  ...
  let rules := ((jsonAt? log [.field "runs", .index 0, .field "tool", .field "driver",
    .field "rules"]).bind (·.getArr?.toOption)).getD #[]
  ensure (!rules.isEmpty) "no rule descriptor carried a helpUri"
  for rule in rules do
    let some uri := (rule.getObjValAs? String "helpUri").toOption
      | throw <| IO.userError "a rule descriptor carried no helpUri"
  ...
  let rulesJson ← expectExit 0 "rules --json" ctx.application #["rules", "--json"]
    (cwd? := some ctx.root)
  let codes := (((← parseJson rulesJson.stdout "rules").getArr?.toOption).getD #[]).filterMap
    fun rule => (rule.getObjValAs? String "code").toOption
...
private def testGithubRoundTrip (ctx : Ctx) : IO Unit := do
  let dup := "module\n\nimport LeanFmt.Basic\nimport LeanFmt.Basic\n"
  let result ← fmt ctx
    #["check", "-", "--stdin-filename", "weird,path:with%signs/A.lean",
      "--output-format", "github"] (input? := some dup)
  let line := (result.stderr.splitOn "\n").head?.getD ""
  let fileValue := match (line.splitOn "file=").tail? with
    | some (after :: _) => (after.splitOn ",").head?.getD ""
    | _ => ""
  let unescaped := fileValue.replace "%3A" ":" |>.replace "%2C" ","
    |>.replace "%0D" "\r" |>.replace "%0A" "\n" |>.replace "%25" "%"
  ensureEq "a GitHub property survives the documented unescaping"
    "weird,path:with%signs/A.lean" unescaped
