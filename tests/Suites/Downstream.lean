...
private def testFacetResolution (ctx : Ctx) : IO Unit := do
  ...
  removeFile? sidecar
  discard <|
      lake ctx #["build", "+Consumer.Basic:leanFmtArtifact"]
...
private def testSilentMessage (ctx : Ctx) : IO Unit := do
  ...
  ensure (output.contains "Unknown identifier") s!"the real error stopped being reported: {output}"
...
private def testInterfaceClosureMode (ctx : Ctx) : IO Unit := do
  ...
  let facets :=
    #["+Consumer.Basic:leanFmtArtifact", "+Consumer.Dup:leanFmtArtifact",
      "+Consumer.Syntax:leanFmtArtifact"]
  ...
  let check (label : String) (args : Array String := #[]) : IO ProcResult := do
    let result ←
      lakeAny ctx (#["exe", "lean-fmt", "check", "--root", "."] ++ args) (env :=
  ...
  try
    ...
    removeFile? config
    try
      discard <| lake ctx #["build"] "post-case restore"
    catch _ =>
      pure ()
