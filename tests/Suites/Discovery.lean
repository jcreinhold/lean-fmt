...
private def selected (ctx : Ctx) (root : System.FilePath) (extra : Array String := #[])
  ...
  let paths :=
    (((jsonAt? data [.field "files"]).bind (·.getArr?.toOption)).getD #[]).filterMap fun file =>
      (file.getObjValAs? String "path").toOption
  return paths.toList.mergeSort (· < ·)

/-- `config showConfig` as parsed JSON. -/
private def showConfig (ctx : Ctx) (root target : System.FilePath)
    (label : String := "config showConfig") (env : Array (String × Option String) := #[]) :
    IO Lean.Json := do
  let result ←
    expectExit 0 label ctx.application
...
private def setting (json : Lean.Json) (key : String) (label : String) : IO String := do
  let some settings :=
    (jsonAt? json [.field "settings"]).bind
      (·.getArr?.toOption) | throw <| IO.userError s!"{label}: no settings"
    ...
    if (entry.getObjValAs? String "key").toOption == some key then
      ...
      | some value =>
        ...
        | .str text =>
          return text
        | other =>
          return other.compress
      | none =>
        throw <| IO.userError s!"{label}: setting {key} has no value"
...
private def originBase (json : Lean.Json) (key : String) (label : String) : IO String := do
  ...
  for entry in settings do
    if (entry.getObjValAs? String "key").toOption == some key then
      let some origin :=
        (entry.getObjValAs? String
            "origin").toOption | throw <| IO.userError s!"{label}: setting {key} has no origin"
...
private def testNested (ctx : Ctx) : IO Unit := do
  ...
  ensureEq "the nested config governs its own directory" "42"
      (←
        setting (← showConfig ctx nested (nested / "lib" / "Lib.lean")) "format.line-width"
            "nested")
  ensureEq "the nested config governs its subdirectories too" "42"
      (←
        setting (← showConfig ctx nested (nested / "lib" / "deep" / "Deep.lean"))
            "format.line-width" "nested")
  ensureEq "a sibling directory keeps the root config" "100"
      (←
        setting (← showConfig ctx nested (nested / "app" / "App.lean")) "format.line-width"
            "nested")
...
private def testExtend (ctx : Ctx) : IO Unit := do
  ...
  ensureEq "the extending file wins a scalar" "42" (← setting shown "format.line-width" "extend")
  ...
  let contributing :=
    (((jsonAt? shown [.field "contributingFiles"]).bind (·.getArr?.toOption)).getD #[]).filterMap
      fun file =>
...
private def testSymlinks (ctx : Ctx) : IO Unit := do
  ...
  discard <|
      expectExit 0 "ln file" "ln" #["-s", "../real/Real.lean", (sym / "dir" / "Link.lean").toString]
  ...
  -- walk is finite.
  ensure (!(symSelected.any (·.contains "loop"))) "the walk descended into a directory symlink loop"
  ...
  let some relative :=
    (linkShow.getObjValAs? String
        "relativePath").toOption | throw <| IO.userError "symlink: no relativePath"
...
private def testIgnoreSources (ctx : Ctx) : IO Unit := do
  ...
  let sources :=
    ((jsonAt? ignShow [.field "ignoreSources"]).bind (·.getArr?.toOption) |>.getD #[]).filterMap
      (·.getStr?.toOption)
...
private def testMigration (ctx : Ctx) : IO Unit := do
  ...
  let result ←
    expectExit 0 "migration check" ctx.application
  ...
  for line in stderr.splitOn "\n"do
    if line.startsWith "phase.discovery_ms=" then
      ...
      | some n =>
        return n
      | none =>
        throw <| IO.userError s!"{label}: unparsable profiling line {line}"
...
private def testLargeTree (ctx : Ctx) : IO Unit := do
  ...
  let deepRun ←
    expectExit 0 "deep config showConfig" ctx.application
        #["config", "show", deepTarget.toString, "--root", big.toString, "--json"] (env :=
        profileEnv)
  ...
  ensureEq "a directory with its own config uses it" "80"
      (←
        setting (← showConfig ctx big (big / "pkg04" / "mod090" / "F0.lean")) "format.line-width"
            "big")
  ...
  let loopRun ←
    expectExit 0 "loop config showConfig" ctx.application
        #["config", "show", deepTarget.toString, "--root", big.toString, "--json"] (env :=
        profileEnv)
