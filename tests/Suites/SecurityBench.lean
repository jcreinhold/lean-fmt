...
private def rows (output : String) : IO (Std.HashMap String Row) := do
  ...
  for line in output.splitOn "\n" |>.filter (· != "") do
    ...
    let some label := fields.head?
      | throw <| IO.userError s!"security-bench emitted a malformed line: {line}"
    let value (key : String) : Option String :=
      (fields.filterMap fun field =>
        if field.startsWith s!"{key}=" then
          some ((field.drop (key.length + 1)).toString)
        else none).head?
    let some bytes := (value "bytes").bind String.toNat?
      | throw <| IO.userError s!"security-bench row {label} carries no bytes: {line}"
    let some ms := (value "ms").bind parseFloat
      | throw <| IO.userError s!"security-bench row {label} carries no ms: {line}"
...
private def gateFlatness (rows : Std.HashMap String Row) (report : Bool := true) : IO Unit := do
  ...
  for label in ["clean-1x", "clean-2x", "clean-4x", "clean-8x"] do
    ...
    if report then IO.println s!"  info {label} {ns} ns/byte"
...
private def testGatesDiscriminate : IO Unit := do
  ...
  let healthy : Std.HashMap String Row := Std.HashMap.ofList [
...
where
  throws (action : IO Unit) : IO Bool :=
    (try action; pure false catch _ => pure true)

end SecurityBench

public def main (args : List String) : IO UInt32 := do
  let root ← repoRoot
  let output ← expectExit 0 "security-bench"
    (root / ".lake" / "build" / "bin" / "lean-fmt-tests").toString #["security-bench"]
    (cwd? := some root) (timeoutMs := some 1800000)
