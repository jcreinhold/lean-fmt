...
private def cpPreserve (source destination : System.FilePath) : IO Unit := do
  discard <| expectExit 0 s!"cp -p {source} {destination}" "cp"
...
private def run (ctx : Ctx) (expected : UInt32) (label : String) (args : Array String)
  ...
  expectExit expected label ctx.app args (cwd? := some ctx.root) (env := env)
...
private def fileMode (path : System.FilePath) : IO String := do
  let result ← expectExit 0 "stat" "sh"
...
private def testLayoutNoNewline (ctx : Ctx) : IO Unit := do
  withRestored ctx.backupLayout ctx.layout do
    ...
    let diff ← run ctx 1 "no-newline diff" (diffArgs "tests/check/Layout.lean")
      ...
      && diff.stdout.contains "+namespace Alpha") "the namespace edit"
    ensure (diff.stdout.contains "-end     Alpha" && diff.stdout.contains "+end Alpha")
      "the end edit"
    ensure (diff.stdout.endsWith
      "mode=diff files=1 findings=0 changed=1 written=0 broken=0 rejected=0 \
...
private def testProjectedHit (ctx : Ctx) : IO Unit := do
  ...
  ensureJsonAt report [.field "files", .index 0, .field "status"] (Lean.toJson "clean")
    "projected"
...
private def testApplicability (ctx : Ctx) : IO Unit := do
  ...
  let edits := (jsonAt? report [.field "files", .index 0, .field "findings", .index 0,
    .field "fix", .field "edits"]).getD (.arr #[])
  ensure ((edits.getArr?.toOption.getD #[]).size > 0) "the fix carries no edits"
  ensure ((jsonAt? report [.field "files", .index 0, .field "findings", .index 0, .field "fix",
    .field "edits", .index 0, .field "range"]).isSome) "the edit carries no range"
...
private def testUnsafeDemotion (ctx : Ctx) : IO Unit := do
  ...
  ensureJsonAt withheld [.field "files", .index 0, .field "status"] (Lean.toJson "clean")
    "withhold"
...
private def testValidatorRejection (ctx : Ctx) : IO Unit := do
  ...
  let diagnostics := ((field (firstFile report) "diagnostics").getArr?.toOption.getD #[])
    |>.toList.map (·.getStr?.toOption.getD "")
...
private def testRulesRegistry (ctx : Ctx) : IO Unit := do
  ...
  ensureEq "the registry order"
    ["FMT001", "FMT002",
     ...
     "FMT003", "FMT004", "FMT005"] codes
  let byCode (code : String) : Lean.Json :=
    (all.find? fun rule => (rule.getObjValAs? String "code").toOption == some code).getD .null
  let flag (code key : String) : Bool :=
    ((field (byCode code) key).getBool?).toOption.getD false
  let text (code key : String) : String :=
    (field (byCode code) key).getStr?.toOption.getD ""
  ...
  ensure (text "FMT013" "category" == "unused" && text "FMT014" "category" == "unused")
    "FMT013/14"
...
private def testDownstream (ctx : Ctx) : IO Unit := do
  ...
  discard <| expectExit 0 "lake update" "lake" #["update"] (cwd? := some downstream)
    (env := lakeEnv) (timeoutMs := some 1800000)
  discard <| expectExit 0 "lake build artifact" "lake"
    #["build", "+Downstream:leanFmtArtifact"] (cwd? := some downstream) (env := lakeEnv)
    (timeoutMs := some 1800000)
  let artifact ← parseJson
    (← IO.FS.readFile (downstream / ".lake" / "build" / "lean-fmt-artifacts" / "Downstream.json"))
    "downstream artifact"
  ...
  let entries := (((jsonAt? artifact [.field "syntaxData", .field "entries"]).getD (.arr #[]))
    |>.getArr?.toOption).getD #[]
  ensure (entries.size > 0) "the downstream projection recorded no syntax entries"
  let status ← runJson ctx 0 "status downstream"
  ...
  let moduleStatuses := (((field status "modules").getArr?.toOption.getD #[]).toList.map
...
private def testCompilerStatus (ctx : Ctx) : IO Unit := do
  ...
  let modulePaths := (((field report "modules").getArr?.toOption.getD #[]).toList.map
    fun m => (m.getObjValAs? String "path").toOption.getD "")
  ensureEq "module paths are not sorted" (modulePaths.mergeSort (· < ·)) modulePaths
  let count (key : String) : Nat :=
    ((field report key).getNum?.toOption.getD 0).mantissa.toNat
  ...
  let moduleStatuses := (((field report "modules").getArr?.toOption.getD #[]).toList.map
    fun m => (m.getObjValAs? String "status").toOption.getD "")
...
private def testRdfImplMixed (ctx : Ctx) : IO Unit := do
  ...
  ensure (((diff.stdout.trimAsciiEnd).toString.endsWith
    "findings=1 changed=1 written=0 broken=0 rejected=0 withheld_unsafe=0 suppressed=0 \
     infrastructure_failures=0")) "the mixed diff trailer"
...
private def testRdfLayout (ctx : Ctx) : IO Unit := do
  ...
  let noselFormat ← runJson ctx 1 "nosel format" (formatCheckArgs "tests/modes/.rdf-layout-nosel.lean")
  ...
  ensureJsonAt noselFormat [.field "files", .index 0, .field "status"]
    (Lean.toJson "would-format") "nosel"
  ...
  let stringFormat ← runJson ctx 1 "string format"
    (formatCheckArgs "tests/modes/.rdf-layout-string.lean")
  ensureJsonAt stringFormat [.field "files", .index 0, .field "status"]
    (Lean.toJson "would-format") "string"
...
private def testCompositionConfluence (ctx : Ctx) : IO Unit := do
  ...
  discard <| run ctx 0 "comp A fix"
    #["fix", "--root", ".", "--no-cache", "tests/modes/.rdf-final-comp-a.lean"]
  let formatA ← runJson ctx 0 "comp A format"
  ...
  let formatB ← runJson ctx 0 "comp B format"
  ...
  let formatB2 ← runJson ctx 0 "comp B format again"
    #["format", "--root", ".", "--json", "--no-cache", "tests/modes/.rdf-final-comp-b.lean"]
  ...
  ensureJsonAt formatB2 [.field "files", .index 0, .field "formatted"] .null
    "order B not a format fixed point"
  for (file, label) in [("tests/modes/.rdf-final-comp-a.lean", "comp A check"),
...
private def testFipExactWrite (ctx : Ctx) : IO Unit := do
  ...
  let first ← runJson ctx 0 "fin exact"
  ...
  let second ← runJson ctx 0 "fin exact again"
    #["format", "--root", ".", "--json", "--no-cache", "tests/modes/.fip-final-exact.lean"]
...
private def testFipWriteRefusals (ctx : Ctx) : IO Unit := do
  ...
  let check ← runJson ctx 1 "fin check"
    (formatCheckArgs "tests/modes/.fip-final-exact.lean")
  ...
  discard <| run ctx 0 "fin check write"
    #["format", "--root", ".", "--json", "--no-cache", "tests/modes/.fip-final-exact.lean"]
  let clean ← runJson ctx 0 "fin check clean"
    (formatCheckArgs "tests/modes/.fip-final-exact.lean")
  ...
  let brokenReport ← runJson ctx 1 "fin broken"
  ...
  let stringReport ← runJson ctx 0 "fin string"
  ...
  ensureJsonAt stringReport [.field "files", .index 0, .field "written"] (Lean.toJson true)
    "fin string"
  ensureEq "the string interior moved"
...
private def testFipStaleAndSelection (ctx : Ctx) : IO Unit := do
  ...
  let staleResult ← run ctx 1 "fin stale"
  ...
  let report ← runJson ctx 0 "fin noarg"
...
private def testRcdFloor (ctx : Ctx) : IO Unit := do
  ...
  for mode in ["format", "fix"] do
    let plain ← run ctx 2 s!"floor {mode}"
    ...
    for setting in ["on", "off"] do
      let configured ← run ctx 2 s!"floor {mode} {setting}"
  ...
  let shown ← runJson ctx 0 "floor show"
...
private def testRcdExplicitExclusion (ctx : Ctx) : IO Unit := do
  ...
  let before ← metadataLine fixture
  let forced ← runJson ctx 0 "forced exclusion"
    #["format", "--root", ".", "--json", "--no-cache", "--config",
  ...
  let plain ← runJson ctx 0 "plain exclusion"
    #["format", "--root", ".", "--json", "--no-cache", "--config",
...
private def testWidthCacheIdentity (ctx : Ctx) : IO Unit := do
  ...
  let narrow ← runJson ctx 1 "width 20"
    #["format", "--check", "--root", ".", "--json", "--config",
  ...
  ensureEq "a width-100 cache entry was served to a width-20 run" ["would-format"]
    (statuses narrow)
  ensure (((jsonAt? wideReport [.field "files", .index 0, .field "formatted"]).getD .null) !=
...
private def testConfigShow (ctx : Ctx) : IO Unit := do
  ...
  let setting (key : String) : Lean.Json :=
    (settings.toList.find? fun entry =>
      (entry.getObjValAs? String "key").toOption == some key).getD .null
...
public def main (args : List String) : IO UInt32 := do
  ...
  withTempDir fun work => do
    ...
    discard <| expectExit 0 "backup" "cp" #["-p", ctx.findings.toString,
      ctx.backupFindings.toString]
    ...
    removeDirAll? ctx.cacheRoot
    discard <| expectExit 0 "lake build fixtures" "lake"
    ...
    let code ← (try runCases "modes" cases args catch error => do
      IO.eprintln (toString error)
      pure 1)
    Modes.cpPreserve ctx.backupFindings ctx.findings
    Modes.cpPreserve ctx.backupLayout ctx.layout
    removeDirAll? ctx.cacheRoot
    for name in [".rdf-layout-nosel.lean", ".rdf-layout-string.lean", ".rdf-layout-tail.lean",
    ...
    for orphan in ← Modes.tmpOrphans ctx.modesDir ".fip-final-" do
      if orphan.contains "lean-fmt-tmp-" then removeFile? (ctx.modesDir / orphan)
    return code
