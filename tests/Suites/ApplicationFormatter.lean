...
private def aLean (ctx : Ctx) : System.FilePath := ctx.work / "A.lean"
private def bLean (ctx : Ctx) : System.FilePath := ctx.work / "B.lean"

private def aSource : String := "module\n\ndef  alpha :Nat:=1\n"
private def bSource : String := "module\n\ndef  beta :Nat:=2\n"
...
private def testPreviewDiffAgreement (ctx : Ctx) : IO Unit := do
  ...
  let preview ← runProc ctx.application
  ...
  let diffRun ← runProc ctx.application
  ...
  let some previewFiles := (jsonAt? previewJson [.field "files"]).bind (·.getArr?.toOption)
    | throw <| IO.userError "preview has no files"
  let some diffFiles := (jsonAt? diffJson [.field "files"]).bind (·.getArr?.toOption)
    | throw <| IO.userError "diff has no files"
  let pathsOf (files : Array Lean.Json) := files.map fun file =>
  ...
  | _ => throw <| IO.userError "preview did not admit exactly two files"
...
private def testCacheService (ctx : Ctx) : IO Unit := do
  discard <| runProc ctx.application
    #["format", "--check", "--root", ctx.root.toString, "--json", (aLean ctx).toString]
    (cwd? := some ctx.root)
  let cached ← runProc ctx.application
    #["format", "--check", "--root", ctx.root.toString, "--json", (aLean ctx).toString]
    (cwd? := some ctx.root)
...
private def testStalePublication (ctx : Ctx) : IO Unit := do
  ...
  let stale ← runProc ctx.application
...
private def testBatchPublication (ctx : Ctx) : IO Unit := do
  writeFile (bLean ctx) bSource
  let written ← expectExit 0 "batch publication" ctx.application
    #["format", "--root", ctx.root.toString, "--no-cache", "--json",
      (aLean ctx).toString, (bLean ctx).toString]
    (cwd? := some ctx.root)
...
private def testRefusal (ctx : Ctx) : IO Unit := do
  ...
  let refusal ← runProc ctx.application
    #["format", "--check", "--root", ctx.root.toString, "--no-cache", "--json", throwing.toString]
    (cwd? := some ctx.root) (env := #[("LEAN_FMT_PROFILE_PHASES", some "1")])
  ensure (refusal.exitCode == 2)
    s!"refusal did not exit 2: {refusal.exitCode}\n{refusal.stderr}"
  ...
  match (jsonAt? report [.field "files", .index 0]).bind
      fun file => (file.getObjValAs? String "status").toOption with
  | some "infrastructure-failure" => pure ()
  | other => throw <| IO.userError s!"refusal file status changed: {repr other}"
...
public def main (args : List String) : IO UInt32 := do
  ...
  -- involved.
  discard <| expectExit 0 "fixture library build" "lake"
    #["build", "lean-fmt", "FormatterAdapterFixtures", "Clean"] (cwd? := some root)
