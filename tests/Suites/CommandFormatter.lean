...
private def testWidth (root setup : System.FilePath) (application : String) (width : Nat) :
    IO Unit := do
  let report ←
    analyzeExact root application setup "tests/fixtures/command-formatter/CoreInput.lean"
        "CoreInput.lean" s!"4:{width}"
...
private def testComments (root setup : System.FilePath) (application : String) : IO Unit := do
  let report ←
    analyzeExact root application setup "tests/fixtures/command-formatter/Comments.lean"
        "Comments.lean" "4:60"
...
private def testSelfModule (root : System.FilePath) (application : String)
  ...
  let report ←
    analyzeExact root application setup "LeanFmt/Formatter/Command.lean"
        "LeanFmt/Formatter/Command.lean" "draft:100"
