...
private def oneFile (report : Lean.Json) (label : String) : IO Lean.Json := do
  ...
  let some file := files[0]? | throw <| IO.userError s!"{label}: report has no file"
...
private def testDocCommentInert (root : System.FilePath) (application : String) : IO Unit := do
  let report ←
    checkJson root application 1 #["tests/fixtures/suppression/DocComment.lean"] "doc-comment"
...
private def testNested (root : System.FilePath) (application : String) : IO Unit := do
  let report ←
    checkJson root application 0 #["tests/fixtures/suppression/Nested.lean"] "nested" (extra :=
        #["--preview", "--select", "FMT011"])
...
private def testCustomCommand (root : System.FilePath) (application : String) : IO Unit := do
  let report ←
    checkJson root application 0 #["tests/fixtures/suppression/Custom.lean"] "custom-command"
...
private def testMovement (root : System.FilePath) (application : String) (work : System.FilePath) :
    IO Unit := do
  ...
  let report ←
    runJson root application 1
        #["format", "--check", "--root", ".", "--json", "--no-cache", scratch.toString]
        "move-format"
...
private def testUnusedFix (root : System.FilePath) (application : String) : IO Unit := do
  ...
  let some find := findings[0]? | throw <| IO.userError "unused-fix: no findings"
  ...
  let some edits :=
    (jsonAt? fix [.field "edits"]).bind
      (·.getArr?.toOption) | throw <| IO.userError "unused-fix: fix has no edits"
  ...
  let cut :=
    String.Pos.Raw.extract source ⟨0⟩ ⟨start⟩ ++ replacement ++
...
private def testMalformedDirective (root : System.FilePath) (application : String) : IO Unit := do
  let report ←
    checkJson root application 1 #["tests/fixtures/suppression/Malformed.lean"] "malformed"
  let file ← oneFile report "malformed"
  let fmts :=
    findingsOf file |>.filter fun finding =>
      (finding.getObjValAs? String "code").toOption == some "FMT901"
  let some find := fmts[0]? | throw <| IO.userError "malformed: no FMT901 finding"
  ensure (fmts.size == 1) "malformed: more than one FMT901"
  ensure
      (((jsonAt? find [.field "fix", .field "applicability"]).bind (·.getStr?.toOption)) ==
        some "display-only")
...
private def testPerFileConfig (root : System.FilePath) (application : String) : IO Unit := do
  let report ←
    checkJson root application 1 #["tests/fixtures/suppression/PerFile.lean"] "per-file" (extra :=
        #["--config", "tests/fixtures/suppression/per-file-ignores.toml"])
      ...
      let cases : Array Case :=
        #[{ name := "doc-comment-inert", run := Suppression.testDocCommentInert root application },
          ...
          { name := "malformed-directive",
            run := Suppression.testMalformedDirective root application },
