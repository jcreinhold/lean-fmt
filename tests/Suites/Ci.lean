...
private def gitQ (args : Array String) (cwd : System.FilePath) (label : String) : IO ProcResult :=
  expectExit 0 label "git"
    (#["-c", "user.email=ci@lean-fmt.invalid", "-c", "user.name=lean-fmt ci"] ++ args)
    (cwd? := some cwd)
...
private def testSarif (ctx : Ctx) : IO Unit := do
  ...
  ensure ((← (ctx.consumer / "findings.sarif").pathExists)
...
private def testCacheRestore (ctx : Ctx) : IO Unit := do
  ...
  discard <| expectExit 0 "tar create" "tar"
    #["-czf", (ctx.work / "cache.tgz").toString, ".lake", ".lean-fmt-cache"]
    (cwd? := some ctx.consumer)
  removeDirAll? (ctx.consumer / ".lake")
  removeDirAll? cacheDir
  discard <| expectExit 0 "tar extract" "tar" #["-xzf", (ctx.work / "cache.tgz").toString]
    (cwd? := some ctx.consumer)
  discard <| lakeAny ctx #["exe", "lean-fmt", "check", "--root", "."]
...
private def testArchiveInstallation (ctx : Ctx) : IO Unit := do
  ...
  discard <| expectExit 0 "git archive" "git"
    #["archive", "--format=tar", "-o", tarballPath.toString, "HEAD"] (cwd? := some ctx.root)
  discard <| expectExit 0 "tar extract" "tar" #["-xf", tarballPath.toString]
    (cwd? := some archive)
  ...
  discard <| expectExit 0 "a clean git archive did not build" "lake" #["build"]
    (cwd? := some archive) (timeoutMs := some 3600000)
  let check ← runProc (archive / ".lake" / "build" / "bin" / "lean-fmt").toString
    #["check", "--root", "."] (cwd? := some archive) (timeoutMs := some 1800000)
...
public def main (args : List String) : IO UInt32 := do
  ...
  withTempDir fun work => do
    let headSha := (← expectExit 0 "git rev-parse" "git" #["rev-parse", "HEAD"]
      (cwd? := some root)).stdout.trimAscii.toString
