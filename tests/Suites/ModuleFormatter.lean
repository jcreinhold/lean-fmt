...
private def metricPos (draft : Lean.Json) (name : String) (label : String) : IO Nat := do
  let some value := natAt? draft [.field "metrics", .field name]
    | throw <| IO.userError s!"{label}: metrics.{name} missing"
  return value

/-- The draft's own region offsets. -/
private def regionOffset (draft : Lean.Json) (name : String) (label : String) : IO Nat := do
  let some value := natAt? draft [.field name]
    | throw <| IO.userError s!"{label}: {name} missing"
  return value
...
private def testTerminalDraft (root : System.FilePath) (application : String)
  ...
  let runDraft : IO Lean.Json := do
    let report ← analyzeExact root application setup terminal.toString terminal.toString "draft:72"
      (viaLakeEnv := true)
  ...
  let some text := (left.getObjValAs? String "text").toOption
    | throw <| IO.userError "terminal: draft text missing"
  ensure (tail.startsWith "#exit" && text.endsWith tail)
    "terminal: the verbatim tail did not ride along"
  let some marks := (jsonAt? left [.field "sourceMap"]).bind (·.getArr?.toOption)
    | throw <| IO.userError "terminal: source map missing"
  ...
  let some entries := (jsonAt? left [.field "headerContract"]).bind (·.getArr?.toOption)
    | throw <| IO.userError "terminal: headerContract missing"
...
private def testOnlyExit (root : System.FilePath) (application : String)
    (work : System.FilePath) (borrowedSetup : System.FilePath) : IO Unit := do
  let onlyExit := work / "OnlyExit.lean"
  writeFile onlyExit "module\n\n#exit\nterminal-only tail\n"
  let report ← analyzeExact root application borrowedSetup onlyExit.toString onlyExit.toString
    "draft" (viaLakeEnv := true)
  ...
  let some text := (draft.getObjValAs? String "text").toOption
    | throw <| IO.userError "only-exit: draft text missing"
  ensure (text == source) "only-exit: draft text is not the source"
  let some marks := (jsonAt? draft [.field "sourceMap"]).bind (·.getArr?.toOption)
    | throw <| IO.userError "only-exit: source map missing"
...
private def testLineEndings (root : System.FilePath) (application : String)
  ...
  let runDraft (name : System.FilePath) (label : String) : IO Lean.Json := do
    let report ← analyzeExact root application setup name.toString name.toString "draft:40"
      (viaLakeEnv := true)
...
private def testPluginSetup (root : System.FilePath) (application : String)
  ...
  let some text := (draft.getObjValAs? String "text").toOption
    | throw <| IO.userError "plugin: draft text missing"
