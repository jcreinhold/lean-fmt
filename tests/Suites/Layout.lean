...
builds the ownership summary; this drives that path over the repository's changing production
corpus. The claim under test — "preserve every comment exactly once" — is decidable rather than
aspirational: the live-syntax summary validates one owner per independently extracted payload.

The suite also checks doc properties (the unit tier's `testDoc`) and the comment summary, decoded
in-process from the analysis envelope.
-/
...
fixture borrows `tests/fixtures/check/Clean.lean`'s setup. The counts are exact because each is a
separate claim about the rule: three same-line comments trail (including the newline-spanning block
Lean's own `chooseNiceTrailStop` would tear in half), one own-line comment leads, one
past-the-last-token comment is file-dangling. -/
