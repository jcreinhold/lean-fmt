...
private partial def firstToken (chars : List Char) (depth : Nat) : String :=
  ...
  | c :: rest =>
    ...
    else if depth > 0 && marker '-' '/' then firstToken (rest.drop 1) (depth - 1)
    else if depth == 0 && marker '-' '-' then firstToken (rest.dropWhile (· != '\n')) 0
    else if depth > 0 then firstToken rest depth
    else if c.isWhitespace then firstToken rest 0
...
private def testNoTrackedArtifacts (root : System.FilePath) : IO Unit := do
  ...
  for path in result.stdout.splitOn "\n" do
    ...
    let forbidden := components.any fun component =>
      component == "Cargo.toml" || component == "Cargo.lock" || component == "target" ||
        component == ".lake" || component == ".lean-fmt-cache" || component.endsWith ".rs"
    ensure (!forbidden)
...
private def testRootExportsNothing (root : System.FilePath) : IO Unit := do
  for line in (← readRepoFile root "LeanFmt.lean").splitOn "\n" do
    let bad := ["import", "def", "structure", "inductive", "class", "abbrev"].any
...
private def testEntryPointSet (root : System.FilePath) : IO Unit := do
  ...
  let expected := #[
    ...
    "tests/Suites/Check.lean", "tests/Suites/CollectionFormatter.lean",
    ...
    "tests/Suites/Discovery.lean", "tests/Suites/FormatSuppression.lean",
    "tests/Suites/Formatter.lean", "tests/Suites/FormatterAdapter.lean",
    "tests/Suites/Imports.lean", "tests/Suites/Incremental.lean", "tests/Suites/Layout.lean",
    "tests/Suites/Lossless.lean", "tests/Suites/Lsp.lean", "tests/Suites/Modes.lean",
    "tests/Suites/ModuleFormatter.lean", "tests/Suites/NativeLayout.lean",
    "tests/Suites/Reporting.lean", "tests/Suites/Scale.lean", "tests/Suites/Semantic.lean",
    "tests/Suites/Style.lean", "tests/Suites/Suppression.lean", "tests/Suites/Syntax.lean",
...
private def testPluginGlobs (root : System.FilePath) : IO Unit := do
  ...
  let block := lines.drop start |>.takeWhile (· != "  ]")
  for forbidden in ["Application", "Cache", "Cli", "Config", "Edit", "Project", "Rules", "Semantic",
      "Service"] do
...
private def testNoLeanServer (root : System.FilePath) : IO Unit := do
  ...
  for line in (← readRepoFile root "LeanFmt/LanguageServer.lean").splitOn "\n" do
    let trimmed := line.trimLeft
    ensure (!(trimmed.startsWith "import Lean.Server" || trimmed.startsWith "import all Lean.Server"))
...
private def testNoLegacyArchitecture (root : System.FilePath) : IO Unit := do
  let forbidden := ["WorkerFleet", "SourceParser", "run_project_fleet", "FleetPlan",
    "libleanshared", "lean-fmt-check-artifacts", "--pinned", "--jobs"]
  let sources := ((← compiledSources root).toList.filter fun path =>
    path.startsWith "LeanFmt/" || path == "Main.lean") ++ ["lakefile.lean"]
...
private def testLinkClosure (root : System.FilePath) : IO Unit := do
  ...
  for image in [".lake/build/bin/lean-fmt",
      ".lake/build/lib/liblean_x2dfmt_LeanFmtCompilerPlugin.dylib"] do
