...
private def explainJson (ctx : Ctx) (code : String) : IO Lean.Json := do
  let result ← expectExit 0 s!"explain {code} --json" ctx.application
    #["explain", code, "--json"] (cwd? := some ctx.root)
...
private def testExamples (ctx : Ctx) : IO Unit := do
  let rulesJson ← expectExit 0 "rules --json" ctx.application #["rules", "--json"]
    (cwd? := some ctx.root)
  let some rules := ((← parseJson rulesJson.stdout "rules").getArr?.toOption)
    | throw <| IO.userError "rules --json is not an array"
  let sorted := rules.qsort fun a b =>
  ...
  for rule in sorted do
    ...
    for index in [0:examples.size] do
      ...
      let report ← runFallback ctx 1
        (#["check", "--root", ".", "--json", "--no-cache"] ++ preview ++
          #["--select", code, relative]) tag
      ...
      let got := (((jsonAt? files[0]! [.field "findings"]).bind (·.getArr?.toOption)).getD #[]).map
      ...
      | some good =>
        if fixable then
          ...
          let recheck ← runFallback ctx 0
            (#["check", "--root", ".", "--json", "--no-cache"] ++ preview ++
              #["--select", code, relative]) tag
          ensureJsonAt (← parseJson recheck.stdout tag) [.field "findings"]
            (Lean.toJson (0 : Nat)) s!"{tag}: fix not idempotent"
        else
          -- A report-only rule may still *show* the corrected form; it must itself be clean.
          writeFile probe (wrap good)
          let report ← runFallback ctx 0
            (#["check", "--root", ".", "--json", "--no-cache"] ++ preview ++
              #["--select", code, relative]) tag
          let got := (((jsonAt? (← parseJson report.stdout tag)
              [.field "files", .index 0, .field "findings"]).bind (·.getArr?.toOption)).getD #[]).map
            fun finding => (finding.getObjValAs? String "code").toOption.getD ""
...
private def testExplainLifecycle (ctx : Ctx) : IO Unit := do
  let expectations : Array (String × UInt32 × String) := #[
    ("FMT001", 0, "FMT001"),
    ("FMT900", 0, "suppressed nothing"),
    ("FMT901", 0, "does not parse as a directive"),
    ("FMT999", 2, "")
  ]
...
private def testDocLinks (ctx : Ctx) : IO Unit := do
  ...
  while true do
    ...
    | _ :: tail :: _ =>
      ...
      unless target.startsWith "http://" || target.startsWith "https://" ||
          target.startsWith "#" do
