...
private def checkJson (ctx : Ctx) (expected : UInt32) (args : Array String) (label : String)
  ...
  let result ←
    expectExit expected label ctx.application args (cwd? := some ctx.root) (env :=
        if fallback then fallbackEnv else #[])
  parseJson result.stdout label

private def oneFile (report : Lean.Json) (label : String) : IO Lean.Json := do
  let some file :=
    jsonAt? report
      [.field "files", .index 0] | throw <| IO.userError s!"{label}: report has no file"
  return file
...
private def testDuplicate (ctx : Ctx) : IO Unit := do
  ...
  let codes :=
    findings.toList.map fun finding =>
      ((finding.getObjValAs? String "code").toOption.getD "",
...
private def testRedundant (ctx : Ctx) : IO Unit := do
  ...
  let select :=
    #["check", "--root", project.toString, "--json", "--no-cache", "--select", "imports"]
  let reported ←
    checkJson ctx 1 (select ++ #[(project / "Demo" / "ViaExported.lean").toString])
        "redundant-exported"
  ...
  let silent ←
    checkJson ctx 0 (select ++ #[(project / "Demo" / "ViaPrivate.lean").toString])
        "redundant-private"
...
private def testOrganizeWrite (ctx : Ctx) : IO Unit := do
  ...
  withRestored ctx path do
      let report ←
        checkJson ctx 0
            #["organize", "--root", ".", "--json", "tests/fixtures/imports/Ordering.lean"]
            "organize-write" (fallback := false)
...
private def testOrganizeWorkers (ctx : Ctx) : IO Unit := do
  ...
  let runOnce (workers : String) : IO (List (String × String) × Array String) := do
    ...
    withRestored ctx ordering <|
        withRestored ctx duplicate do
          let report ←
            checkJson ctx 0
          ...
          let statuses :=
            (files.toList.map fun file =>
                  (((file.getObjValAs? String "path").toOption).getD "",
                    ((file.getObjValAs? String "status").toOption).getD "")).mergeSort
              (·.1 < ·.1)
...
private def testOrganizeCanonical (ctx : Ctx) : IO Unit := do
  ...
  let dry ←
    checkJson ctx 1 #["organize", "--check", "--root", ".", "--json", "--config", config, target]
  ...
  withRestored ctx path do
      let report ←
        checkJson ctx 0 #["organize", "--root", ".", "--json", "--config", config, target]
            "organize-canonical" (fallback := false)
...
private def testVerdictCache (ctx : Ctx) : IO Unit := do
  ...
  withRestored ctx ordering do
      ...
      IO.FS.writeFile ordering <|
          original.replace "import LeanFmt.Digest"
            "import LeanFmt.NoSuchModule\nimport LeanFmt.Digest"
      let first ←
        expectExit 1 "verdict-store" ctx.application #["organize", "--root", ".", "--json", target]
            (cwd? := some ctx.root) (env := profileEnv)
      ensure (childrenOf first.stderr == 1)
          s!"verdict-store: the first organize did not validate through the frontend:\n{first.stderr}"
      let second ←
        expectExit 1 "verdict-serve" ctx.application #["organize", "--root", ".", "--json", target]
            (cwd? := some ctx.root) (env := profileEnv)
  ...
  withRestored ctx ordering do
      let organized ←
        expectExit 0 "publish" ctx.application #["organize", "--root", ".", "--json", target]
            (cwd? := some ctx.root) (env := profileEnv)
      ensure (childrenOf organized.stderr == 1)
          s!"publish: organize did not validate its candidate:\n{organized.stderr}"
      let fresh ←
        runProc ctx.application #["check", "--root", ".", "--json", "--no-cache", target] (cwd? :=
            some ctx.root) (env := fallbackEnv)
      let served ←
        runProc ctx.application #["check", "--root", ".", "--json", target] (cwd? := some ctx.root)
            (env := profileEnv)
...
private def testFixFormatSplit (ctx : Ctx) : IO Unit := do
  ...
  ensureEq "fix-conflict: FMT003 fix not applied at original coordinates" ["import LeanFmt.Basic"]
      fixImports
  ensure (afterFix.endsWith "0  \n") "fix-conflict: fix touched the layout -- it must not"
  seed
  let formatReport ←
    checkJson ctx 1 #["format", "--check", "--root", ".", "--json", "--no-cache", conflict.toString]
        "format-conflict"
