...
verify_artifacts() {
  ...
  LEAN_NUM_THREADS=1 lake exe lean-fmt-tests \
    verify-plugin-artifact LocalSyntax tests/compiler/LocalSyntax.lean "$expected"
  LEAN_NUM_THREADS=1 lake exe lean-fmt-tests \
    verify-facet-artifact "$artifact" tests/compiler/LocalSyntax.lean "$expected" "$expected_hash"
}
...
LEAN_NUM_THREADS=1 lake exe lean-fmt-tests verify-facet-artifact \
  "$shadow_dir/exact.json" tests/compiler/LocalSyntax.lean true "$exact_hash"
...
# Rule configuration is part of the owning module trace and payload.
LEAN_NUM_THREADS=1 lake -R -KleanFmtTrailingWhitespace=false build +LocalSyntax:leanFmtArtifact
...
# A real plugin binary change must invalidate the module job through Lake's plugin dependency,
# rather than through a formatter-maintained identity field.
LEAN_NUM_THREADS=1 lake -R build +LocalSyntax:leanFmtArtifact
...
path = Path(sys.argv[1])
source = path.read_text()
old = 'descr := "report and repair trailing horizontal whitespace"'
new = 'descr := "probe: report and repair trailing horizontal whitespace"'
if source.count(old) != 1:
...
if LEAN_NUM_THREADS=1 lake exe lean-fmt-tests \
    verify-facet-artifact "$artifact" tests/compiler/LocalSyntax.lean true \
...
if LEAN_NUM_THREADS=1 lake exe lean-fmt-tests \
    verify-plugin-artifact LocalSyntax tests/compiler/LocalSyntax.lean true; then
