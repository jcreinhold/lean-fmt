...
public partial def awaitResponse (id : Nat) : IpcM Answer := do
  ...
  | .response responseId value =>
    if responseId == (id : RequestID) then
      return .result value
    else
      awaitResponse id
...
public def initializeSession (root : String) (options : Json := Json.mkObj []) : IpcM Answer := do
  request 0 "initialize"
      (Json.mkObj
        [("processId", Json.null), ("rootUri", Json.str s!"file://{root}"),
          ("capabilities", Json.mkObj []), ("initializationOptions", options)])
...
public def range (startLine startCharacter stopLine stopCharacter : Nat) : Json :=
  Json.mkObj
    [("start", position startLine startCharacter), ("end", position stopLine stopCharacter)]
...
public partial def readFrameJson : IpcM Json := do
  ...
  | .ok json =>
    return json
  | .error message =>
    throw <| IO.userError s!"the server sent a frame that is not JSON: {message}"
