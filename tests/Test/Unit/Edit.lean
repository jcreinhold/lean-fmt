...
private def ensureRejected (source : String) (findings : Array Finding) (accept : PatchError → Bool)
...
private def testEdits : IO Unit := do
  ...
  let ordered :=
    #[findingWithEdit { start := 0, stop := 1 } "A", findingWithEdit { start := 1, stop := 2 } "B"]
    ...
    for stop in boundaries do
      if start <= stop then
        for replacement in replacements do
          let patch ← requirePatch propertySource #[findingWithEdit { start, stop } replacement]
...
private def testFixAllAdversarial : IO Unit := do
    ...
    requirePatch "abcd"
        #[findingWithEdits
            #[{ range := { start := 0, stop := 1 }, replacement := "X" },
              { range := { start := 2, stop := 3 }, replacement := "Y" }]
            .safe "MULTI"]
    ...
    preparePatch "abcd"
      #[findingWithEdits
          #[{ range := { start := 0, stop := 2 }, replacement := "X" },
            { range := { start := 1, stop := 3 }, replacement := "Y" }]
          .safe "MULTI"] with
  | .error (.conflict left right _ _) =>
    ensure (left == "MULTI" && right == "MULTI")
        "an intra-fix conflict lost the fix's own provenance"
  | _ =>
    throw <| IO.userError "overlapping edits within one fix were accepted"
  ...
  | _ =>
    throw <| IO.userError "an overlapping syntax/import fix pair was accepted"
