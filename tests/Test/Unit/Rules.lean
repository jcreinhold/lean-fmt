...
private def testSourceSecurityProperties : IO Unit := do
  ...
  let oracle (s : String) : Array (String × Nat × Nat) :=
    Id.run do
      ...
      for c in s.toList do
        if bidiSet.contains c.toNat then
          acc := acc.push ("FMT002", pos, pos + c.utf8Size)
        pos := pos + c.utf8Size
...
private def testCatalogInvariants : IO Unit := do
  ...
  for info in infos do
    ...
    if info.lifecycle == .deprecated then
      ...
      let some r :=
        info.replacement? | throw <| IO.userError s!"deprecated rule {info.code} has no replacement"
  ...
  for info in infos do
    if info.lifecycle == .preview then
      ...
      | none =>
        throw <| IO.userError s!"preview rule {info.code} states no path out of preview"
      | some p =>
        ensure (!p.isEmpty) s!"preview rule {info.code} has an empty path out of preview"
    else
      ensure info.previewPath?.isNone s!"non-preview rule {info.code} carries a path out of preview"
...
private def testApplicability : IO Unit := do
  ...
  for (a, wire) in
    #[(Applicability.safe, "safe"), (.unsafe, "unsafe"), (.displayOnly, "display-only")] do
        ...
        | .ok decoded => decoded == a
        | .error _ => false)
        s!"applicability did not round-trip: {wire}"
  ensure
      (match (Lean.fromJson? (.str "bogus") : Except String Applicability) with
      | .error _ => true
      | _ => false)
      "an unknown applicability wire value was accepted"
  ...
  let plan : RulePlan :=
    { selected := #["FMT011", "FMT012"], perFileIgnores := #[], extendSafe := #["FMT011"],
      extendUnsafe := #["FMT012"] }
  ensure (plan.effectiveApplicability "FMT011" .unsafe == .safe) "extend-safe-fixes did not promote"
  ensure (plan.effectiveApplicability "FMT011" .safe == .safe)
      "promotion changed an already-safe fix"
  ensure (plan.effectiveApplicability "FMT012" .safe == .unsafe)
      "extend-unsafe-fixes did not demote"
  ...
  let demote : RulePlan :=
    { selected := #["FMT011"], perFileIgnores := #[], extendSafe := #[],
      extendUnsafe := #["FMT011"] }
  let projected :=
    demote.findings "A.lean" #[findingWithEdit { start := 0, stop := 1 } "" .safe "FMT011"]
  ...
  match
    preparePatch "abc"
      #[findingWithEdit { start := 0, stop := 2 } "x" .safe "RULE_A",
        findingWithEdit { start := 1, stop := 3 } "y" .safe "RULE_B"] with
  | .error error =>
  ...
  | .ok _ =>
    throw <| IO.userError "overlapping fixes were accepted"
...
private def testEngineTiers : IO Unit := do
  ...
  let mixed := runRulesOf registry syntaxFacts
      ...
      (mixed.map (fun finding => (finding.range.start, finding.range.stop)) == #[(0, 3), (0, 11)])
  ...
  ensure (probeSyntax.tier == .syntax && probeSource.tier == .source)
      "Rule.tier is not RuleImpl.tier"
  ...
  -- semantic `--select`. This asserts the shape: all three tiers now ship.
      ...
      (ruleRegistry.any (·.tier == .source) && ruleRegistry.any (·.tier == .syntax) &&
...
private def testUnbuiltClassification : IO Unit := do
  let unbuilt :=
    #["/proj/Foo.lean:1:0: error: failed to open file \
    '/proj/.lake/build/lib/lean/Mathlib/Data/Nat/Basic.olean': No such file or directory"]
  ensure (unbuiltDependency? unbuilt == some "/proj/.lake/build/lib/lean/Mathlib/Data/Nat/Basic")
  ...
  let unknown :=
    #["/proj/Foo.lean:1:0: error: unknown module prefix 'NoSuchModule'\n\n\
