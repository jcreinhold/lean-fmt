...
private def testSemanticArtifact : IO Unit := do
  ...
  match decoded with
  | .ok actual =>
    ensure (actual == fixtureSemanticArtifact) "v9 semantic artifact JSON round trip failed"
  | .error message =>
    throw <| IO.userError s!"v9 semantic artifact decode failed: {message}"
  -- The plugin producer emits `semantic = none`; that shape is valid and round-trips too.
  ensure (fixtureArtifact.semantic.isNone)
      "the plugin-shaped fixture already carried a semantic fact"
  ...
  | .ok actual =>
    ensure (actual == fixtureArtifact) "v9 semantic = none artifact JSON round trip failed"
  | .error message =>
    throw <| IO.userError s!"v9 semantic = none artifact decode failed: {message}"
  ...
  | .ok _ =>
    throw <| IO.userError "the v9 decoder retained the deleted v4 payload shape"
  | .error _ =>
    pure ()
...
private def testSemanticRules : IO Unit := do
  ...
  let expect : Array (String × String × Nat × Nat) :=
    #[("FMT012", "`oldName` is deprecated", 0, 4), ("FMT013", "unused variable `x`", 5, 6),
  ...
  for (code, message, start, stop) in expect do
    ...
    | #[f] =>
      ...
      ensure (f.range.start == start && f.range.stop == stop)
          s!"{code} did not preserve the diagnostic range"
    ...
    | other =>
      throw <| IO.userError s!"expected exactly one {code} finding, got {other.size}"
  ...
  ensure (surfacedDiagnosticKinds.size == 4)
      "surfacedDiagnosticKinds no longer lists exactly the four rules"
  ensure
      ((findings.filter
            (fun f => #["FMT012", "FMT013", "FMT014", "FMT015"].contains f.code)).size ==
        4)
...
private def testOwnedDeprecationFix : IO Unit := do
  ...
  let fmt014 (occurrences : Array DeprecatedOccurrence) : IO Finding := do
    ...
    | #[f] =>
      return f
    | other =>
      throw <| IO.userError s!"expected exactly one FMT012 finding, got {other.size}"
  ...
  let fixable : DeprecatedOccurrence :=
    { range := depRange, declName := "oldName", newName? := some "newName", since? := some "1.0",
      text? := none, fixable := true }
  ...
  | none =>
    throw <| IO.userError "a fixable deprecation occurrence attached no fix"
...
private def testSemanticCaps : IO Unit := do
  ...
  -- The empty demand is served by an occurrence-bearing entry (superset), orthogonal to the tier.
      ...
      "the cheap sub-facts are not a subset of the full capability set"
