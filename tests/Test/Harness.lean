...
public structure Selection where
  ...
  `index - 1` modulo `count`. One-based because `--part 1/3` naming the first third is the
    ...
    | [index, count] =>
      ...
      | some index, some count =>
        if index == 0 || count == 0 || index > count then
          .error s!"--part expects 1 ≤ INDEX ≤ COUNT, got: {spec}"
        else
          .ok { selection with part := some (index, count) }
      | _, _ =>
        .error s!"--part expects INDEX/COUNT, got: {spec}"
    | _ =>
      .error s!"--part expects INDEX/COUNT, got: {spec}"
...
public def runCases (label : String) (cases : Array Case) (args : List String) : IO UInt32 := do
  let selection ←
    ...
    | .error error =>
      do
        IO.eprintln error
        IO.eprintln s!"usage: {label} [--list] [--filter SUBSTRING] [--part INDEX/COUNT]"
        return 2
