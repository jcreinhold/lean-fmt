...
structure Patch where private mk ::
  source : Digest
  output : String
  edits : Array Edit
  inverse : Array Edit

def Patch.formatted (patch : Patch) : String :=
  patch.output

def Patch.editCount (patch : Patch) : Nat :=
  patch.edits.size

def Patch.changed (patch : Patch) : Bool :=
  !patch.edits.isEmpty
...
private def editLess (left right : IndexedEdit) : Bool :=
  if left.edit.range.start != right.edit.range.start then
    ...
    if left.edit.range.stop != right.edit.range.stop then
...
private def sortedEdits (edits : Array ProvenancedEdit) : Array IndexedEdit :=
  (edits.mapIdx fun index item =>
        { index, code := item.code, findingRange := item.findingRange, edit := item.edit }).qsort
    editLess
...
private def assemble (source : String) (ordered : Array IndexedEdit) :
    Except PatchError (String × Array Edit) := do
