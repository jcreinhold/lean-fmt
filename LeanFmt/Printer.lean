...
repository (110,164 nodes, `evidence/01-projection-shape.txt`), 37,377 nodes (33.9%) carry no token at
...
leaves beneath it and there are none. For 16,874 of them (15.3% of all nodes) the parent also has direct
...
2. **The conservative path reads bytes, not the tree.** Empty nodes contribute no bytes, so re-emitting
   a command's byte extent is unaffected by all 16,874 ambiguous placements. It is the only path whose
...
token-bearing children, index order agrees with byte order (0 violations over 110,164 nodes). -/
