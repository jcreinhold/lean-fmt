...
repository (100,631 nodes, `evidence/01-projection-shape.txt`), 34,648 nodes (34.4%) carry no token at
...
leaves beneath it and there are none. For 15,673 of them (15.6% of all nodes) the parent also has direct
...
1. **Node order is index order, never range order.** `collect` pushes a node's placeholder at
   ...
   siblings ascend in arg order. Sorting children by range would be correct for the 65.6% that carry
...
2. **The conservative path reads bytes, not the tree.** Empty nodes contribute no bytes, so re-emitting
   a command's byte extent is unaffected by all 15,673 ambiguous placements. It is the only path whose
...
token-bearing children, index order agrees with byte order (0 violations over 100,631 nodes). -/
...
`none` is the *absent syntax* case and is the common one: 34.4% of nodes are empty
