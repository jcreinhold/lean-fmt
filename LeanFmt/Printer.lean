...
repository (79,309 nodes, `evidence/01-projection-shape.txt`), 27,839 nodes (35.1%) carry no token at
...
leaves beneath it and there are none. For 12,642 of them (15.9% of all nodes) the parent also has direct
...
1. **Node order is index order, never range order.** `collect` pushes a node's placeholder at
   ...
   siblings ascend in arg order. Sorting children by range would be correct for the 64.9% that carry
...
2. **The conservative path reads bytes, not the tree.** Empty nodes contribute no bytes, so re-emitting
   a command's byte extent is unaffected by all 12,642 ambiguous placements. It is the only path whose
...
token-bearing children, index order agrees with byte order (0 violations over 79,309 nodes). -/
...
/-- Do these two offsets lie on the same source line?

Asked of the *interval*, not of two columns, because that is the question the collapse guard needs and
it needs no answer for either endpoint on its own. `columnOf` folds the whole prefix from byte zero;
this folds only the span between, so the guard stays linear in the command rather than in the module.
-/
private def sameLine (normalized : String) (a b : Nat) : Bool :=
  !(sliceNormalized normalized a b).contains '\n'
...
`none` is the *absent syntax* case and is the common one: 35.1% of nodes are empty
...
private def Tree.parts (tree : Tree) (node : Nat) : Array Part := Id.run do
  ...
  -- measures 15.9% of nodes to be exactly that ambiguous).
