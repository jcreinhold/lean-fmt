...
repository (42,599 nodes, `evidence/01-projection-shape.txt`), 15,251 nodes (35.8%) carry no token at
...
leaves beneath it and there are none. For 6,617 of them (15.5% of all nodes) the parent also has direct
...
2. **The conservative path reads bytes, not the tree.** Empty nodes contribute no bytes, so re-emitting
   a command's byte extent is unaffected by all 6,617 ambiguous placements. It is the only path whose
...
token-bearing children, index order agrees with byte order (0 violations over 42,599 nodes). -/
