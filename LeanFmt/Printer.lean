...
repository (43,840 nodes, `evidence/01-projection-shape.txt`), 15,654 nodes (35.7%) carry no token at
...
leaves beneath it and there are none. For 6,823 of them (15.6% of all nodes) the parent also has direct
...
2. **The conservative path reads bytes, not the tree.** Empty nodes contribute no bytes, so re-emitting
   a command's byte extent is unaffected by all 6,823 ambiguous placements. It is the only path whose
...
token-bearing children, index order agrees with byte order (0 violations over 43,840 nodes). -/
