...
instance : Lean.ToJson DirectiveScope := ⟨fun s => Lean.toJson s.toIndex⟩
instance : Lean.FromJson DirectiveScope := ⟨fun j => do
...
private def isAlnum (c : Char) : Bool := c.isAlphanum
...
private def parseBody (afterSigil : String) : Except String (DirectiveScope × Option (Array String)) := do
  let rest := trim afterSigil
  -- Split verb from an optional bracketed selector list. More than one '[' is malformed.
  let (verbStr, bracket?) ←
    match rest.splitOn "[" with
    | [only] => pure (trim only, none)
    | [verb, sel] => pure (trim verb, some sel)
    | _ => throw "more than one '[' in suppression directive"
  let scope ←
    match verbStr with
    | "ignore" => pure DirectiveScope.line
    | "ignore-next" => pure DirectiveScope.nextItem
    | "ignore-file" => pure DirectiveScope.file
    | other => throw s!"unknown suppression verb '{other}'"
  match bracket? with
  ...
  | some sel =>
    ...
    let (codesStr, tail) ←
      ...
      | [codes, tail] => pure (codes, tail)
      | [_] => throw "unclosed '[' in suppression directive"
      | _ => throw "more than one ']' in suppression directive"
...
private def lineStart (bytes : ByteArray) (pos : Nat) : Nat := Id.run do
  let mut i := pos
...
private def lineStop (bytes : ByteArray) (pos : Nat) : Nat := Id.run do
  let mut i := pos
...
private def directiveScope (src : LosslessSource) (bytes : ByteArray)
    (scope : DirectiveScope) (comment : SourceRange) : SourceRange :=
  match scope with
  | .file => ⟨0, bytes.size⟩
...
private def removalRange (bytes : ByteArray) (comment : SourceRange) : SourceRange :=
  ...
  let precededOnlyByWhitespace := Id.run do
    let mut i := ls
    while i < comment.start do
      let b := bytes[i]!
      unless b == 0x20 || b == 0x09 do return false
      i := i + 1
    return true
...
private def rewriteDirective (scope : DirectiveScope) (codes : Array String) : String :=
  let verb := match scope with | .line => "ignore" | .nextItem => "ignore-next" | .file => "ignore-file"
...
def apply (facts : SuppressionFacts) (bytes : ByteArray) (findings : Array Finding) : Outcome := Id.run do
  let mut kept := #[]
