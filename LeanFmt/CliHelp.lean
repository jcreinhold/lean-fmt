...
private def optRoot : HelpEntry := ⟨"--root PATH", "Lake project root (default: .)"⟩
private def optConfig : HelpEntry := ⟨"--config PATH", "explicit lean-fmt.toml"⟩
private def optChanged : HelpEntry := ⟨"--changed", "select only files differing from HEAD, plus untracked"⟩
private def optChangedSince : HelpEntry :=
  ⟨"--changed-since REV", "select only files this branch changed since REV"⟩
private def optStaged : HelpEntry := ⟨"--staged", "select only files staged for commit"⟩
...
private def optIgnore : HelpEntry := ⟨"--ignore SELECTOR", "deactivate a rule/category/all (repeatable)"⟩
private def optPreview : HelpEntry := ⟨"--preview", "unlock preview (experimental) rules"⟩

private def optFixable : HelpEntry :=
  ⟨"--fixable SELECTOR", "restrict which rules' fixes apply (repeatable)"⟩
private def optUnfixable : HelpEntry := ⟨"--unfixable SELECTOR", "withhold a rule's fix (repeatable)"⟩
private def optExtendFixable : HelpEntry :=
  ⟨"--extend-fixable SEL", "add to the fixable set without replacing (repeatable)"⟩
private def optUnsafeFixes : HelpEntry := ⟨"--unsafe-fixes", "apply unsafe fixes too (default: safe only)"⟩
private def optUnsafeFixesPreview : HelpEntry :=
  ⟨"--unsafe-fixes", "preview unsafe fixes too (default: safe only)"⟩

private def optJson : HelpEntry := ⟨"--json", "deterministic JSON output"⟩
...
private def optOutputFormatDiff : HelpEntry :=
  ⟨"--output-format FMT", "text|json (default: text); concise/github/sarif/junit describe findings, and a patch carries none"⟩
private def optOutputFile : HelpEntry :=
  ⟨"--output-file PATH", "write the report to PATH atomically instead of stdout"⟩
private def optStatistics : HelpEntry := ⟨"--statistics", "write aggregate statistics to stderr"⟩

private def optNoCache : HelpEntry := ⟨"--no-cache", "neither read nor write result cache entries"⟩
private def optWorkers : HelpEntry :=
  ⟨"--workers N", "run up to N frontend children in parallel (default: LEAN_NUM_THREADS, else the machine's core count); the report is byte-identical at any N"⟩
...
private def commandHelps : Array CommandHelp := #[
  { command := "check"
    ...
    description := "Report the selected rules' findings for each file, and nothing else: a \
    ...
    sections := #[
      ...
      fixSelectionSection optUnsafeFixesPreview,
      outputSection optOutputFormat,
  ...
  { command := "format"
    ...
    description := "Render each file's canonical layout and publish it in place, atomically. \
  ...
  { command := "diff"
    ...
    description := "Print the patch `format` would publish, file by file. Never writes. Exit 0 no \
    ...
    sections := #[
      targetSection,
      ruleSelectionSection,
      outputSection optOutputFormatDiff,
      executionSection true,
      stdinSection false
    ]
    notes := #[
  ...
  { command := "fix"
    ...
    description := "Apply the admitted rules' fixes in place, atomically, at each file's original \
    ...
    sections := #[
      targetSection,
      ruleSelectionSection,
      fixSelectionSection optUnsafeFixes,
      outputSection optOutputFormat,
  ...
  { command := "lsp"
    ...
    description := "Speak LSP on stdio — document formatting, range formatting, code actions, and \
    ...
    sections := #[
      ("server options:", #[optRoot, optConfig, optSelect, optIgnore, optPreview, optUnsafeFixes,
        optDebounceMs])
    ] },
  { command := "organize"
  ...
  { command := "rules"
    ...
    description := "Print every rule in the registry, one row each: code, category, lifecycle, \
  ...
  { command := "explain"
    ...
    description := "Print one rule's full description. A retired code prints its disposition; a \
  ...
  { command := "docs"
    ...
    description := "Write docs/rules/{index,FMT###}.md fresh from the rule registry. `--check` \
  ...
  { command := "clean"
    ...
    description := "Remove .lean-fmt-cache/ under the root. Exit 0 whether or not it existed, 2 \
      failure."
  ...
  { command := "compiler"
    ...
    description := "Inspect and build the compiler plugin's per-module syntax artifact. The \
    ...
    usage := #[
      "lean-fmt compiler setup [--json]",
      "lean-fmt compiler status [--root PATH] [--json]",
    ...
    notes := #[
      "`build` prints Lake's own progress, so `--json` does not apply to it."
    ] },
  { command := "config"
    summary := "inspect the effective configuration for a file"
    description := "`config show PATH` prints the settings in force for one file and where each \
