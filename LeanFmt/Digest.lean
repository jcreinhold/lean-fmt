...
identities. -/
structure Digest where private mk ::
  hex : String
...
private def padded (input : ByteArray) : ByteArray :=
  Id.run do
...
private def compress (state : Array UInt32) (words : Array UInt32) : Array UInt32 :=
  Id.run do
    let mut a := state[0]!
    let mut b := state[1]!
    let mut c := state[2]!
    ...
    return #[state[0]! + a, state[1]! + b, state[2]! + c, state[3]! + d, state[4]! + e,
        state[5]! + f, state[6]! + g, state[7]! + h]

private def hashBytes (input : ByteArray) : Array UInt32 :=
  Id.run do
...
def parse? (value : String) : Option Digest :=
  if
      value.length == 64 &&
        value.toList.all fun char => ('0' ≤ char && char ≤ '9') || ('a' ≤ char && char ≤ 'f') then
    some (.mk value)
  else none

instance : ToString Digest :=
  ⟨Digest.hex⟩

instance : Lean.ToJson Digest :=
  ⟨fun digest => .str digest.hex⟩
...
instance : Lean.FromJson Digest where
  fromJson? json := do
    ...
    | some digest =>
      pure digest
    | none =>
      throw "expected a 64-digit lowercase SHA-256 digest"
