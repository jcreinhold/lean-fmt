...
**Audience: anyone deciding whether to depend on this, and whoever maintains it next.** It answers one question — what
happens when Lean moves — and states plainly which parts a machine does and which parts a person does.

lean-fmt is maintained by one person. That is a fact worth knowing before you depend on it, so the rest of this document
is about what has been automated so that it does not depend on that person having a free evening.
...
table to consult and no version to choose.

This is forced rather than chosen. lean-fmt loads your project's `.olean`s, and those load only in the compiler that
wrote them, so there is exactly one correct answer per toolchain and no second axis to carry. Lake is told the same
thing directly — the package sets `fixedToolchain := true`, so a consumer on a different toolchain fails at resolution
rather than somewhere deep in a build.
...
tag — Lean's cadence is faster than most fixes need. `lake-manifest.json` records whatever you resolved, so a SHA pin is
as reproducible as a tag pin.

**Nothing is distributed as a binary.** lean-fmt is built from source against your compiler, which is what the compiler
plugin and cache facet require anyway. Releases through `v0.7.1` shipped tarballs; those tags still exist and still work.
...
rather than as a note in a summary somebody reads next month. Most Lean releases are that kind.

## What a person decides

The automation deliberately stops short of four things, because each needs a judgement that a green check cannot carry:
