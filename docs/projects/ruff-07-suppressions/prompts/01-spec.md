...
# Specify suppression grammar and scope
...
- Write `results/01-spec.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-07-suppressions`.
- Run `git diff --check` and read all output before marking RSP-SPEC verified.
