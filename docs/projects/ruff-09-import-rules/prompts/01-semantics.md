...
# Characterize Lean import-order and redundancy semantics
...
- Write `results/01-semantics.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-09-import-rules`.
- Run `git diff --check` and read all output before marking RIR-SPEC verified.
