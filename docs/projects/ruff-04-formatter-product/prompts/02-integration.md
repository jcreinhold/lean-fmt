...
# Integrate formatter output and atomic publication
...
- Write `results/02-integration.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-04-formatter-product`.
- Run `git diff --check` and read all output before marking RFP-IMPL verified.
