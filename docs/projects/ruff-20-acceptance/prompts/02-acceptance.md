...
- Write `results/02-acceptance.md` with exact commands, raw outputs or evidence locators, measurements,
...
- Stop on any resource excursion, missing file, crash, semantic mismatch, or misleading performance claim.
- Completion requires a clean worktree-ready evidence matrix and all structural checks.
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-20-acceptance`.
- Run `git diff --check` and read all output before marking RCP-ACCEPT verified.
