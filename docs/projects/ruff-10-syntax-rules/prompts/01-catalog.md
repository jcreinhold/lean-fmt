...
# Freeze the syntax-rule catalog from corpus evidence
...
- Write `results/01-catalog.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-10-syntax-rules`.
- Run `git diff --check` and read all output before marking RYR-SPEC verified.
