...
# Test integrations from clean sources

## Task

Deliver **RDI-FINAL**: Run shell completion syntax checks, a temporary pre-commit repository, CI
command simulations, package/install smoke for the first-party VS Code, Neovim, and Emacs adapters,
generic LSP launch, and clean `git archive` installation smoke.
...
- Record platform/tool limitations honestly.
...
- Run `git diff --check` and read all output before marking RDI-FINAL verified.
