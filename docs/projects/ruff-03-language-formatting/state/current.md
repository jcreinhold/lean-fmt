...
this repository *and* on 62 modules of foreign Lean, **914 of the corpus's 1021 commands take a cited
canonical layout** — `namespace` (33), `end` (35), `open` (13), 2 `section`, and the shell of 832 of
...
- **Coverage is counted by the printer, because byte identity cannot see it and the corpus cannot
  ...
  commands actually laid out, and `tests/printer/run.sh` floors the corpus total: **914 of 1021**, and `members=` the shells claimed
...
- **Two independent measurements of coverage differ by one, and the per-module comparison says why —
  ...
  declarations claimable; its formula counts 913 = 832 + 33 `namespace` + 35 `end` + 13 `open`, while
  the printer, in Lean, lays out 914. Two causes in opposite directions, measured per module in
...
- **That 914 commands take the layout and all 20 modules stay byte-identical is what proves the shell
