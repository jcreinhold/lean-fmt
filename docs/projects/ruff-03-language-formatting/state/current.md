...
**`RLF-NOTATION` is verified** (`results/06-notation-facts.md`), and it is the first output bytes phase 2
changes. The printer consumes the `ruff-05b` `v4` notation-spacing fact — one immutable field threaded
into `Tree` as a kind-keyed `Std.HashMap`, no `Environment`, no frontend import — and emits declared
spacing: `1+2` → `1 + 2`, `8  +  9` → `8 + 9`, and the corpus notation `3⊗4` → `3 ⊗4`. The finding is
...
this repository *and* on 62 modules of foreign Lean, **927 of the corpus's 1035 commands take a cited
canonical layout** — `namespace` (34), `end` (36), `open` (16), 2 `section`, and the shell of 840 of
860 declarations, less the one shell in `Imports.lean` the runtime guards refuse — **all 28 module
...
- **Coverage is counted by the printer, because byte identity cannot see it and the corpus cannot
  ...
  commands actually laid out, and `tests/printer/run.sh` floors the corpus total: **927 of 1035**, and `members=` the shells claimed
...
- **Two independent measurements of coverage differ by one, and the per-module comparison says why —
  ...
  structural half of the printer's predicate in Python against the same projection and finds 840 of 860
  declarations claimable; its formula counts 926 = 840 + 34 `namespace` + 36 `end` + 16 `open`, while
  the printer, in Lean, lays out 927. Two causes in opposite directions, measured per module in
...
- **That 927 commands take the layout and all 20 modules stay byte-identical is what proves the shell
