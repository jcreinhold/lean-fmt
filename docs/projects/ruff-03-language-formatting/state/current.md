...
this repository *and* on 62 modules of foreign Lean, **937 of the corpus's 1044 commands take a cited
canonical layout** — `namespace` (34), `end` (36), `open` (15), 2 `section`, and the shell of 851 of
871 declarations, less the one shell in `Imports.lean` the runtime guards refuse — **all 28 module
...
- **Coverage is counted by the printer, because byte identity cannot see it and the corpus cannot
  ...
  commands actually laid out, and `tests/printer/run.sh` floors the corpus total: **937 of 1044**, and `members=` the shells claimed
...
- **Two independent measurements of coverage differ by one, and the per-module comparison says why —
  ...
  structural half of the printer's predicate in Python against the same projection and finds 851 of 871
  declarations claimable; its formula counts 936 = 851 + 34 `namespace` + 36 `end` + 15 `open`, while
  the printer, in Lean, lays out 937. Two causes in opposite directions, measured per module in
...
- **That 937 commands take the layout and all 20 modules stay byte-identical is what proves the shell
