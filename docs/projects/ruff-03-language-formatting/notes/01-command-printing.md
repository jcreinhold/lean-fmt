...
| nodes whose subtree contains no token at all | **37,519 — 33.9%** |
| …of those, whose parent also has direct token children | **16,932 — 15.3% of all nodes** |
...
**Therefore arg order cannot be recovered from positions.** For 16,932 of 110,683 nodes, an empty
...
- **Node-children are read in arg order, never sorted by range.** Arg order is guaranteed by
  `collect`; range order is wrong for 15.7% of nodes and *silently* wrong, which is worse.
...
- **The conservative fallback reads tokens, not the tree.** This is the load-bearing consequence of
  ...
  with their trivia is unaffected by all 16,932 ambiguous placements. Unknown syntax round-trips
...
**1045 commands in 9 distinct kinds** (`evidence/01-projection-shape.txt`; the counts below move as the
project grows and are re-read from the probe rather than maintained by hand).

| kind | count | owner |
| --- | --- | --- |
| `Lean.Parser.Command.declaration` | 872 | `RLF-COMMANDS` — the shell and its members; see below |
...
the same census over the frozen mathlib sample — 2734 commands against this repository's 1045 — and the
...
sections" is in `prompts/01-commands.md`; the census above shows 0 in 1045 commands here and 181 in the
