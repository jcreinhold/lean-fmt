...
| nodes whose subtree contains no token at all | **17,806 — 35.5%** |
| …of those, whose parent also has direct token children | **7,877 — 15.7% of all nodes** |
...
**Therefore arg order cannot be recovered from positions.** For 7,877 of 50,221 nodes, an empty
...
- **The conservative fallback reads tokens, not the tree.** This is the load-bearing consequence of
  ...
  with their trivia is unaffected by all 7,877 ambiguous placements. Unknown syntax round-trips
...
**506 commands in 6 distinct kinds** (`evidence/01-projection-shape.txt`; the counts below move as the
project grows and are re-read from the probe rather than maintained by hand).

| kind | count | owner |
| --- | --- | --- |
| `Lean.Parser.Command.declaration` | 437 | `RLF-COMMANDS` — the shell and its members; see below |
...
the same census over the frozen mathlib sample — 2734 commands against this repository's 506 — and the
...
sections" is in `prompts/01-commands.md`; the census above shows 0 in 506 commands here and 181 in the
