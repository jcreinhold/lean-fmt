...
| nodes whose subtree contains no token at all | **27,839 — 35.1%** |
| …of those, whose parent also has direct token children | **12,642 — 15.9% of all nodes** |
...
**Therefore arg order cannot be recovered from positions.** For 12,642 of 79,309 nodes, an empty
...
- **The conservative fallback reads tokens, not the tree.** This is the load-bearing consequence of
  ...
  with their trivia is unaffected by all 12,642 ambiguous placements. Unknown syntax round-trips
...
**737 commands in 6 distinct kinds** (`evidence/01-projection-shape.txt`; the counts below move as the
project grows and are re-read from the probe rather than maintained by hand).

| kind | count | owner |
| --- | --- | --- |
| `Lean.Parser.Command.declaration` | 634 | `RLF-COMMANDS` — the shell and its members; see below |
...
the same census over the frozen mathlib sample — 2734 commands against this repository's 737 — and the
...
sections" is in `prompts/01-commands.md`; the census above shows 0 in 737 commands here and 181 in the
