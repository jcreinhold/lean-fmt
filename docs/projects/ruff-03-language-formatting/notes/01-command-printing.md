...
| nodes whose subtree contains no token at all | **37,236 — 33.8%** |
| …of those, whose parent also has direct token children | **16,838 — 15.3% of all nodes** |
...
**Therefore arg order cannot be recovered from positions.** For 16,838 of 110,155 nodes, an empty
...
- **The conservative fallback reads tokens, not the tree.** This is the load-bearing consequence of
  ...
  with their trivia is unaffected by all 16,838 ambiguous placements. Unknown syntax round-trips
...
**1037 commands in 9 distinct kinds** (`evidence/01-projection-shape.txt`; the counts below move as the
project grows and are re-read from the probe rather than maintained by hand).

| kind | count | owner |
| --- | --- | --- |
| `Lean.Parser.Command.declaration` | 862 | `RLF-COMMANDS` — the shell and its members; see below |
...
the same census over the frozen mathlib sample — 2734 commands against this repository's 1037 — and the
...
sections" is in `prompts/01-commands.md`; the census above shows 0 in 1037 commands here and 181 in the
