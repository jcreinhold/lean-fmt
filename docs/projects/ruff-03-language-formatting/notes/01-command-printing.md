...
| nodes whose subtree contains no token at all | **19,042 — 35.4%** |
| …of those, whose parent also has direct token children | **8,483 — 15.8% of all nodes** |
...
**Therefore arg order cannot be recovered from positions.** For 8,483 of 53,760 nodes, an empty
...
- **The conservative fallback reads tokens, not the tree.** This is the load-bearing consequence of
  ...
  with their trivia is unaffected by all 8,483 ambiguous placements. Unknown syntax round-trips
...
**541 commands in 6 distinct kinds** (`evidence/01-projection-shape.txt`; the counts below move as the
project grows and are re-read from the probe rather than maintained by hand).

| kind | count | owner |
| --- | --- | --- |
| `Lean.Parser.Command.declaration` | 468 | `RLF-COMMANDS` — the shell and its members; see below |
...
the same census over the frozen mathlib sample — 2734 commands against this repository's 541 — and the
...
sections" is in `prompts/01-commands.md`; the census above shows 0 in 541 commands here and 181 in the
