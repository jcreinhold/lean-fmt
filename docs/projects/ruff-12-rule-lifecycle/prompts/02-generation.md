...
# Implement registry-derived CLI and docs
...
- Write `results/02-generation.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-12-rule-lifecycle`.
- Run `git diff --check` and read all output before marking RRL-IMPL verified.
