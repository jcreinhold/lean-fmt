...
# Implement the renderer and comment attachment
...
- Write `results/02-engine.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-02-layout-core`.
- Run `git diff --check` and read all output before marking RLC-IMPL verified.
