...
# Freeze source-rule specifications
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-08-source-rules`.
- Run `git diff --check` and read all output before marking RSR-SPEC verified.
