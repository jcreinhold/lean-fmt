...
# Install durable performance gates

## Task

Deliver **RPR-FINAL**: Add fast per-commit micro/representative checks, scheduled heavier sample checks, saved raw profiles, digest reuse, variance policy, and a result note accepting or rejecting concurrency.
...
- Write `results/03-regressions.md` with exact commands, raw outputs or evidence locators, measurements,
...
- Do not encode flaky wall-time thresholds without calibration.
- No full mathlib run.
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-19-performance`.
- Run `git diff --check` and read all output before marking RPR-FINAL verified.
