...
- **Next is RPR-FINAL** (`prompts/03-regressions.md`): per-commit gates from growth ratios and phase values (never wall times -- see the 5x arm-A swing), scheduled sample checks, saved raw profiles, digest reuse, variance policy, and the concurrency note, which `results/02-optimize.md` already supplies as reject.
