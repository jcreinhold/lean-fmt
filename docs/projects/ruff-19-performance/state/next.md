...
- **Remaining, in order.** (1) The adversarial `PositionIndex`-build fixture inherited from `ruff-15`, and `phase.positions_ms` measured against it -- it reads 0 ms on every workload so far. (2) A `formatter-integrated-built` workload. (3) The two-session concurrency test, last.
