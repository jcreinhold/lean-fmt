...
- **RPR-FINAL in progress.** `tests/performance/run.sh` exists and passes: §1 warm run fully cache-served (34/34, zero `exact_child`, zero `exact_setup`), §2 gate G3, §3 the zero-phase regression gate, §4 cold/warm report identity. **Gate G3 was recalibrated and the correction matters**: the unaccounted remainder is a *constant* ~51 ms (51/51/67/51/50 over walls of 453-1,225 ms) -- process startup nothing brackets -- so G3 as a bare percentage is workload-length-dependent (0.5% of mathlib-sample, 11% of self) and the published 95.1%/97.2% are exactly what that constant predicts. The suite bounds the remainder at 250 ms instead.
- **RPR-FINAL remaining**: negative-test every gate (a gate that cannot fail is not a gate); the scheduled heavier checks; saved raw profiles with digest reuse; the written variance policy; `results/03-regressions.md`; and add `performance` to the suite list in `CLAUDE.md`.
- **Was: next is RPR-FINAL** (`prompts/03-regressions.md`): per-commit gates from growth ratios and phase values (never wall times -- see the 5x arm-A swing), scheduled sample checks, saved raw profiles, digest reuse, variance policy, and the concurrency note, which `results/02-optimize.md` already supplies as reject.
