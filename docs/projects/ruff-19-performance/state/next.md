...
- **Remaining, in order.** (1) Watch and LSP profiling. (2) The adversarial `PositionIndex`-build fixture inherited from `ruff-15`, and `phase.positions_ms` measured against it -- it reads 0 ms on every workload so far. (3) A `formatter-integrated-built` workload. (4) The two-session concurrency test, last.
