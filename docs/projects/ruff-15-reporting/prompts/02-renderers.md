...
# Implement pure deterministic renderers
...
- Write `results/02-renderers.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-15-reporting`.
- Run `git diff --check` and read all output before marking RRF-IMPL verified.
