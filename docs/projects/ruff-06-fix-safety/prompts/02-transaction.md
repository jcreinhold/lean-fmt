...
# Implement applicability-aware planning and publication
...
- Write `results/02-transaction.md` with exact commands, raw outputs or evidence locators, measurements,
...
- From the KanProofs tool environment, run the generic stack structural checker and
  `write_next.py --check` for `docs/projects/ruff-06-fix-safety`.
- Run `git diff --check` and read all output before marking RFX-IMPL verified.
