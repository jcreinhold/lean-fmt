...
| part 3/3 stalls after `layout` passes: `syntax` and `check` start, then ~18 min of silence until the 20-minute step timeout | CI runs 30702577635, 30704463051 | twice, identical signature | instrumented — the runner now carries a stall watchdog ("still running: NAME (Ns)" every 15 s past the first minute), so the next recurrence names the suite instead of the step |
